#ifndef AsdSDPCUNCPData_H
#define AsdSDPCUNCPData_H

#include <stdlib.h>
#ifdef  AsdSDPCUNCP_INTEGRITY_STRING
#undef  AsdSDPCUNCP_INTEGRITY_STRING
#endif
#define AsdSDPCUNCP_INTEGRITY_STRING        ""

#ifdef  AsdSDPCUNCP_CRC_STRING
#undef  AsdSDPCUNCP_CRC_STRING
#endif
#define AsdSDPCUNCP_CRC_STRING "2533072684"
...
//**************** Start AsdSDPCUNCPData Definition ****************
//                 Loss Prevention - NCPS

//                 SDPCUNCPData.h generated by AceGen2 ver. 2.5.09

class AsdSDPCUNCPData
{
  public:
  ...
  public:
    AsdSDPCUNCPData ()  // Constructors
    {
        InitializeSessionData(&m_Sess);                      // InitializeSessionData declared in bcAce2.h
        m_Sess.pFileName = "SDPCUNCP.ace";                       // This variable is fixed at gen time
        m_Sess.pProtocol = "TLU0";                           // This variable is fixed at gen time
        m_Sess.pIntegrityString = AsdSDPCUNCP_INTEGRITY_STRING;  // This variable is fixed at gen time
    ...
    RWCString HostLUName (void)
    {
        ...
        AddXMLField(&m_Sess, m_Sess.pIntegrityTag, AsdSDPCUNCP_INTEGRITY_STRING, NULL);
    ...
    long Execute (void)
    {
        ...
        AddXMLField(&m_Sess, m_Sess.pCRCTag, AsdSDPCUNCP_CRC_STRING, NULL);
        AddXMLRecord(&m_Sess, m_Sess.pDataTag, -1);       // DATA
        CTSHeader.Save(&m_Sess);
        CSDNCCOII.Save(&m_Sess);
        CSDNCCOIO.Save(&m_Sess);
        CTSMDA.Save(&m_Sess);
        ...
        if ((ret = ExecuteXMLTransaction(&m_Sess)) == 0)
        {
            if (GetXMLRecord(&m_Sess, m_Sess.pAceTag, NULL, NULL))
            {
                ...
                {
                    if(GetXMLRecord(&m_Sess, "SDPCUNCP", NULL, NULL))
                    {
                        CTSHeader.Load(&m_Sess);
                        CSDNCCOII.Load(&m_Sess);
                        CSDNCCOIO.Load(&m_Sess);
                        CTSMDA.Load(&m_Sess);
    
                        CloseXMLRecord(&m_Sess, "SDPCUNCP");
                    }
    ...
    //**************** Start AsdSDNCCOII Definition ****************
    //                 SERVICE DELIVERY REQUEST RECORD TO NCPS                     
    ...
    class AsdSDNCCOII
    {
      private:
        ...
        char      m_szSuppressBuff[100];
        bool      m_bSdToNcps;
        char      m_szSdToNcps[1784+1];
    
      public:
        long      m_lIndex;

        AsdSDNCCOII ()  // Constructor
        {
            m_bHostInitOverride = false;
            strcpy(m_szHostInitOverride, "");
            m_bReceived = false;
        ...
        //    INQUIRY MESSAGE SENT TO NCPS.
        long lSdToNcps () { return 1784; };
        RWCString SdToNcps (void)
        {
            return m_szSdToNcps;
        }
        long SdToNcps (const RWCString& rwc)
        {
            return SetString(rwc, m_szSdToNcps, sizeof(m_szSdToNcps), &m_bSdToNcps);
        }
        
        bool HasDataToSave (void)
        {
            if (m_bHostInitOverride) return true;
            if (m_bSdToNcps) return true;
            return false;
        }
        ...
        void Save (SESSIONDATA *pSess)
        {
            if (HasDataToSave())
            {
                AddXMLRecord(pSess, "SDNCCOII", m_lIndex);
                AddXMLField(pSess, "HostInitOverride", m_szHostInitOverride, &m_bHostInitOverride);
                AddXMLField(pSess, "SdToNcps", m_szSdToNcps, &m_bSdToNcps);
                AddXMLCloseRecord(pSess, "SDNCCOII");  // ACE
            };
        }

        void Load (SESSIONDATA *pSess)
        {
            if (GetXMLRecord(pSess, "SDNCCOII", m_lIndex, &m_bReceived))
            {
                GetXMLField(pSess, "SdToNcps", m_szSdToNcps);
                CloseXMLRecord(pSess, "SDNCCOII");
            };
        }
    };

    AsdSDNCCOII CSDNCCOII;
    AsdSDNCCOII *SDNCCOII;
    
    //**************** End AsdSDNCCOII Definition ****************

    
    //**************** Start AsdSDNCCOIO Definition ****************
    //                 SERVICE DELIVERY RESPONSE RECORD FROM NCPS                  
    ...
    class AsdSDNCCOIO
    {
      private:
        ...
        char      m_szSuppressBuff[100];
        bool      m_bNcpsToSd;
        char      m_szNcpsToSd[5981+1];
    
      public:
        long      m_lIndex;

        AsdSDNCCOIO ()  // Constructor
        {
            m_bHostInitOverride = false;
            strcpy(m_szHostInitOverride, "");
            m_bReceived = false;
        ...
        long lNcpsToSd () { return 5981; };
        RWCString NcpsToSd (void)
        {
            return m_szNcpsToSd;
        }
        long NcpsToSd (const RWCString& rwc)
        {
            return SetString(rwc, m_szNcpsToSd, sizeof(m_szNcpsToSd), &m_bNcpsToSd);
        }
        
        bool HasDataToSave (void)
        {
            if (m_bHostInitOverride) return true;
            if (m_bNcpsToSd) return true;
            return false;
        }
        ...
        void Save (SESSIONDATA *pSess)
        {
            if (HasDataToSave())
            {
                AddXMLRecord(pSess, "SDNCCOIO", m_lIndex);
                AddXMLField(pSess, "HostInitOverride", m_szHostInitOverride, &m_bHostInitOverride);
                AddXMLField(pSess, "NcpsToSd", m_szNcpsToSd, &m_bNcpsToSd);
                AddXMLCloseRecord(pSess, "SDNCCOIO");  // ACE
            };
        }

        void Load (SESSIONDATA *pSess)
        {
            if (GetXMLRecord(pSess, "SDNCCOIO", m_lIndex, &m_bReceived))
            {
                GetXMLField(pSess, "NcpsToSd", m_szNcpsToSd);
                CloseXMLRecord(pSess, "SDNCCOIO");
            };
        }
    };

    AsdSDNCCOIO CSDNCCOIO;
    AsdSDNCCOIO *SDNCCOIO;
    
    //**************** End AsdSDNCCOIO Definition ****************
...
//**************** End AsdSDPCUNCPData Definition ****************

typedef AsdSDPCUNCPData *LPSDPCUNCP;
typedef AsdSDPCUNCPData::AsdTSHeader *LPSDPCUNCP_TSHeader;
typedef AsdSDPCUNCPData::AsdSDNCCOII *LPSDPCUNCP_SDNCCOII;
typedef AsdSDPCUNCPData::AsdSDNCCOIO *LPSDPCUNCP_SDNCCOIO;
typedef AsdSDPCUNCPData::AsdTSMDA *LPSDPCUNCP_TSMDA;
typedef AsdSDPCUNCPData::AsdTSMDA::AsdMdaTrmMsg *LPSDPCUNCP_TSMDA_MdaTrmMsg;
typedef AsdSDPCUNCPData::AsdTSMDA::AsdMdaTrmMsg::AsdMdaMsg *LPSDPCUNCP_TSMDA_MdaTrmMsg_MdaMsg;
#endif    // AsdSDPCUNCPData_H
