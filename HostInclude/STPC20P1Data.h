#ifndef AsdSTPC20P1Data_H
#define AsdSTPC20P1Data_H

#include <stdlib.h>
#ifdef  AsdSTPC20P1_INTEGRITY_STRING
#undef  AsdSTPC20P1_INTEGRITY_STRING
#endif
#define AsdSTPC20P1_INTEGRITY_STRING        ""

#ifdef  AsdSTPC20P1_CRC_STRING
#undef  AsdSTPC20P1_CRC_STRING
#endif
#define AsdSTPC20P1_CRC_STRING "3792505053"
...
//**************** Start AsdSTPC20P1Data Definition ****************
//                 ST Field Maintenance, svInterestDisposition 

//                 STPC20P1Data.h generated by AceGen2 ver. 2.5.09

class AsdSTPC20P1Data
{
  public:
  ...
  public:
    AsdSTPC20P1Data ()  // Constructors
    {
        InitializeSessionData(&m_Sess);                      // InitializeSessionData declared in bcAce2.h
        m_Sess.pFileName = "STPC20P1.ace";                       // This variable is fixed at gen time
        m_Sess.pProtocol = "TLU0";                           // This variable is fixed at gen time
        m_Sess.pIntegrityString = AsdSTPC20P1_INTEGRITY_STRING;  // This variable is fixed at gen time
    ...
    RWCString HostLUName (void)
    {
        ...
        AddXMLField(&m_Sess, m_Sess.pIntegrityTag, AsdSTPC20P1_INTEGRITY_STRING, NULL);
    ...
    long Execute (void)
    {
        ...
        if ((ret = ExecuteXMLTransaction(&m_Sess)) == 0)
        {
            if (GetXMLRecord(&m_Sess, m_Sess.pAceTag, NULL, NULL))
            {
                ...
                {
                    if(GetXMLRecord(&m_Sess, "STPC20P1", NULL, NULL))
                    {
                        CTSHeader.Load(&m_Sess);
                        CSDSTCMMI.Load(&m_Sess);
                        CSDST15MI.Load(&m_Sess);
                        CSTTRMNT1.Load(&m_Sess);
                        CTSMDA.Load(&m_Sess);
    
                        CloseXMLRecord(&m_Sess, "STPC20P1");
                    }
    ...
    //**************** Start AsdSTTRMNT1 Definition ****************
    //                 FIELD MAINTENANCE DAG                                       
    
    //    STTRMNT1.map
    //    generated by Dsm2Ace ver. 2.3.03
    //    on 06/18/02 13:37:31
    class AsdSTTRMNT1
    {
      private:
        bool      m_bHostInitOverride;
      ...
      public:
        ...
        AsdSTTRMNT1 ()  // Constructor
        {
            ...
            {
                Record[lOccurs]  = &CRecord[lOccurs];
                CRecord[lOccurs].m_lIndex = lOccurs;
            }
        }
        ...
        bool HasDataToSave (void)
        {
            ...
            for (lOccurs = 0; lOccurs < 10; lOccurs++)
            {
                if(CRecord[lOccurs].HasDataToSave())
                {
                    return true;
                }
            }
        ...
        void Save (SESSIONDATA *pSess)
        {
            if (HasDataToSave())
            {
                AddXMLRecord(pSess, "STTRMNT1", m_lIndex);
                AddXMLField(pSess, "HostInitOverride", m_szHostInitOverride, &m_bHostInitOverride);
                long lOccurs;
                for (lOccurs = 0; lOccurs < 10; lOccurs++)
                {
                    CRecord[lOccurs].Save(pSess);
                }
                AddXMLCloseRecord(pSess, "STTRMNT1");  // ACE
            };
        }

        void Load (SESSIONDATA *pSess)
        {
            if (GetXMLRecord(pSess, "STTRMNT1", m_lIndex, &m_bReceived))
            {
                long lOccurs;
                for (lOccurs = 0; lOccurs < 10; lOccurs++)
                {
                    CRecord[lOccurs].Load(pSess);
                }
                CloseXMLRecord(pSess, "STTRMNT1");
            };
        }
        
        //**************** Start AsdRecord Definition ****************
        //                 MAINT TRAN RECORD
        ...
        class AsdRecord
        {
          ...
          public:
            ...
            //    TRANSACTION CUSTOMER GENERATED INDICATOR
            long lCustGen () { return 1; };
            RWCString CustGen (void)
            {
                return m_szCustGen;
            ...
            //    MAINT TRAN FIELD NUMBER
            long lFldNbr () { return 3; };
            RWCString FldNbr (void)
            {
                return m_szFldNbr;
            }
            long FldNbr (const RWCString& rwc)
            {
                return SetString(rwc, m_szFldNbr, sizeof(m_szFldNbr), &m_bFldNbr);
            }
            
            //    MAINT TRAN OFFICER INITIALS
            long lOffInitials () { return 3; };
            RWCString OffInitials (void)
            {
                return m_szOffInitials;
            ...
            //    MAINT TRAN NEW DATA
            long lNewData () { return 40; };
            RWCString NewData (void)
            {
                return m_szNewData;
            }
            long NewData (const RWCString& rwc)
            {
                return SetString(rwc, m_szNewData, sizeof(m_szNewData), &m_bNewData);
            }
            ...
            //    MAINTENANCE AFTER POSTING INDICATOR
            long lAfterPost () { return 1; };
            RWCString AfterPost (void)
            {
                return m_szAfterPost;
            ...
            {
                if (m_bCustGen) return true;
                if (m_bFldNbr) return true;
                if (m_bOffInitials) return true;
                if (m_bNewData) return true;
                if (m_bAction) return true;
                if (m_bAfterPost) return true;
                return false;
            }
            ...
            void Save (SESSIONDATA *pSess)
            {
                if (HasDataToSave())
                {
                    AddXMLRecord(pSess, "Record", m_lIndex);
                    AddXMLField(pSess, "CustGen", m_szCustGen, &m_bCustGen);
                    AddXMLField(pSess, "FldNbr", m_szFldNbr, &m_bFldNbr);
                    AddXMLField(pSess, "OffInitials", m_szOffInitials, &m_bOffInitials);
                    AddXMLField(pSess, "NewData", m_szNewData, &m_bNewData);
                    AddXMLField(pSess, "Action", m_szAction, &m_bAction);
                    AddXMLField(pSess, "AfterPost", m_szAfterPost, &m_bAfterPost);
                    AddXMLCloseRecord(pSess, "Record");  // ACE
                };
            }

            void Load (SESSIONDATA *pSess)
            {
                if (GetXMLRecord(pSess, "Record", m_lIndex, &m_bReceived))
                {
                    GetXMLField(pSess, "CustGen", m_szCustGen);
                    GetXMLField(pSess, "FldNbr", m_szFldNbr);
                    GetXMLField(pSess, "OffInitials", m_szOffInitials);
                    GetXMLField(pSess, "NewData", m_szNewData);
                    GetXMLField(pSess, "Action", m_szAction);
                    GetXMLField(pSess, "AfterPost", m_szAfterPost);
                    CloseXMLRecord(pSess, "Record");
                };
            }
        };

        AsdRecord  CRecord[10];
        AsdRecord *Record[10];
        
        //**************** End AsdRecord Definition ****************
        
    };

    AsdSTTRMNT1 CSTTRMNT1;
    AsdSTTRMNT1 *STTRMNT1;
    
    //**************** End AsdSTTRMNT1 Definition ****************
...
//**************** End AsdSTPC20P1Data Definition ****************

typedef AsdSTPC20P1Data *LPSTPC20P1;
typedef AsdSTPC20P1Data::AsdTSHeader *LPSTPC20P1_TSHeader;
typedef AsdSTPC20P1Data::AsdSDSTCMMI *LPSTPC20P1_SDSTCMMI;
typedef AsdSTPC20P1Data::AsdSDST15MI *LPSTPC20P1_SDST15MI;
typedef AsdSTPC20P1Data::AsdSTTRMNT1 *LPSTPC20P1_STTRMNT1;
typedef AsdSTPC20P1Data::AsdSTTRMNT1::AsdRecord *LPSTPC20P1_STTRMNT1_Record;
typedef AsdSTPC20P1Data::AsdTSMDA *LPSTPC20P1_TSMDA;
typedef AsdSTPC20P1Data::AsdTSMDA::AsdMdaTrmMsg *LPSTPC20P1_TSMDA_MdaTrmMsg;
typedef AsdSTPC20P1Data::AsdTSMDA::AsdMdaTrmMsg::AsdMdaMsg *LPSTPC20P1_TSMDA_MdaTrmMsg_MdaMsg;
#endif    // AsdSTPC20P1Data_H
