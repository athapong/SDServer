#ifndef AsdWVPCBR14Data_H
#define AsdWVPCBR14Data_H

#include <stdlib.h>
#ifdef  AsdWVPCBR14_INTEGRITY_STRING
#undef  AsdWVPCBR14_INTEGRITY_STRING
#endif
#define AsdWVPCBR14_INTEGRITY_STRING        ""

#ifdef  AsdWVPCBR14_CRC_STRING
#undef  AsdWVPCBR14_CRC_STRING
#endif
#define AsdWVPCBR14_CRC_STRING "4041259357"
...
//**************** Start AsdWVPCBR14Data Definition ****************
//                 Hostoffline

//                 WVPCBR14Data.h generated by AceGen2 ver. 2.5.09

class AsdWVPCBR14Data
{
  public:
  ...
  public:
    AsdWVPCBR14Data ()  // Constructors
    {
        InitializeSessionData(&m_Sess);                      // InitializeSessionData declared in bcAce2.h
        m_Sess.pFileName = "WVPCBR14.ace";                       // This variable is fixed at gen time
        m_Sess.pProtocol = "TLU0";                           // This variable is fixed at gen time
        m_Sess.pIntegrityString = AsdWVPCBR14_INTEGRITY_STRING;  // This variable is fixed at gen time
    ...
    RWCString HostLUName (void)
    {
        ...
        AddXMLField(&m_Sess, m_Sess.pIntegrityTag, AsdWVPCBR14_INTEGRITY_STRING, NULL);
    ...
    long Execute (void)
    {
        ...
        AddXMLField(&m_Sess, m_Sess.pCRCTag, AsdWVPCBR14_CRC_STRING, NULL);
        AddXMLRecord(&m_Sess, m_Sess.pDataTag, -1);       // DATA
        CTSHeader.Save(&m_Sess);
        CWVBR14RC.Save(&m_Sess);
        CTSMDA.Save(&m_Sess);
        ...
        if ((ret = ExecuteXMLTransaction(&m_Sess)) == 0)
        {
            if (GetXMLRecord(&m_Sess, m_Sess.pAceTag, NULL, NULL))
            {
                ...
                {
                    if(GetXMLRecord(&m_Sess, "WVPCBR14", NULL, NULL))
                    {
                        CTSHeader.Load(&m_Sess);
                        CWVBR14RC.Load(&m_Sess);
                        CTSMDA.Load(&m_Sess);
    
                        CloseXMLRecord(&m_Sess, "WVPCBR14");
                    }
    ...
    //**************** Start AsdWVBR14RC Definition ****************
    //                 WVPCBR14 HOST OFFLINE CHECK                                 
    ...
    class AsdWVBR14RC
    {
      private:
        ...
        bool      m_bReceived;
        char      m_szSuppressBuff[100];
        bool      m_bSystemReturnCode;
        char      m_szSystemReturnCode[4+1];
        bool      m_bSystemResponse;
        char      m_szSystemResponse[96+1];
    
      public:
        long      m_lIndex;

        AsdWVBR14RC ()  // Constructor
        {
            m_bHostInitOverride = false;
            strcpy(m_szHostInitOverride, "");
            m_bReceived = false;
            m_lIndex = -1;
            m_bSystemReturnCode = false;
            strcpy(m_szSystemReturnCode, "");
            m_bSystemResponse = false;
            strcpy(m_szSystemResponse, "");
        }
        ...
        //    DUMMY DAG FOR WVPCBR14
        long lSystemReturnCode () { return 4; };
        RWCString SystemReturnCode (void)
        {
            return m_szSystemReturnCode;
        }
        long SystemReturnCode (const RWCString& rwc)
        {
            return SetString(rwc, m_szSystemReturnCode, sizeof(m_szSystemReturnCode), &m_bSystemReturnCode);
        }
        
        //    DUMMY DAG FOR WVPCBR14
        long lSystemResponse () { return 96; };
        RWCString SystemResponse (void)
        {
            return m_szSystemResponse;
        }
        long SystemResponse (const RWCString& rwc)
        {
            return SetString(rwc, m_szSystemResponse, sizeof(m_szSystemResponse), &m_bSystemResponse);
        }
        
        bool HasDataToSave (void)
        {
            if (m_bHostInitOverride) return true;
            if (m_bSystemReturnCode) return true;
            if (m_bSystemResponse) return true;
            return false;
        }
        ...
        void Save (SESSIONDATA *pSess)
        {
            if (HasDataToSave())
            {
                AddXMLRecord(pSess, "WVBR14RC", m_lIndex);
                AddXMLField(pSess, "HostInitOverride", m_szHostInitOverride, &m_bHostInitOverride);
                AddXMLField(pSess, "SystemReturnCode", m_szSystemReturnCode, &m_bSystemReturnCode);
                AddXMLField(pSess, "SystemResponse", m_szSystemResponse, &m_bSystemResponse);
                AddXMLCloseRecord(pSess, "WVBR14RC");  // ACE
            };
        }

        void Load (SESSIONDATA *pSess)
        {
            if (GetXMLRecord(pSess, "WVBR14RC", m_lIndex, &m_bReceived))
            {
                GetXMLField(pSess, "SystemReturnCode", m_szSystemReturnCode);
                GetXMLField(pSess, "SystemResponse", m_szSystemResponse);
                CloseXMLRecord(pSess, "WVBR14RC");
            };
        }
    };

    AsdWVBR14RC CWVBR14RC;
    AsdWVBR14RC *WVBR14RC;
    
    //**************** End AsdWVBR14RC Definition ****************
...
//**************** End AsdWVPCBR14Data Definition ****************

typedef AsdWVPCBR14Data *LPWVPCBR14;
typedef AsdWVPCBR14Data::AsdTSHeader *LPWVPCBR14_TSHeader;
typedef AsdWVPCBR14Data::AsdWVBR14RC *LPWVPCBR14_WVBR14RC;
typedef AsdWVPCBR14Data::AsdTSMDA *LPWVPCBR14_TSMDA;
typedef AsdWVPCBR14Data::AsdTSMDA::AsdMdaTrmMsg *LPWVPCBR14_TSMDA_MdaTrmMsg;
typedef AsdWVPCBR14Data::AsdTSMDA::AsdMdaTrmMsg::AsdMdaMsg *LPWVPCBR14_TSMDA_MdaTrmMsg_MdaMsg;
#endif    // AsdWVPCBR14Data_H
