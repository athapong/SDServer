#ifndef AsdTWCASHAJData_H
#define AsdTWCASHAJData_H

#include <stdlib.h>
#ifdef  AsdTWCASHAJ_INTEGRITY_STRING
#undef  AsdTWCASHAJ_INTEGRITY_STRING
#endif
#define AsdTWCASHAJ_INTEGRITY_STRING        ""

#ifdef  AsdTWCASHAJ_CRC_STRING
#undef  AsdTWCASHAJ_CRC_STRING
#endif
#define AsdTWCASHAJ_CRC_STRING "323122861"
...
//**************** Start AsdTWCASHAJData Definition ****************
//                 Cash Adjustment

//                 TWCASHAJData.h generated by AceGen2 ver. 2.5.09

class AsdTWCASHAJData
{
  public:
  ...
  public:
    AsdTWCASHAJData ()  // Constructors
    {
        InitializeSessionData(&m_Sess);                      // InitializeSessionData declared in bcAce2.h
        m_Sess.pFileName = "TWCASHAJ.ace";                       // This variable is fixed at gen time
        m_Sess.pProtocol = "TLU0";                           // This variable is fixed at gen time
        m_Sess.pIntegrityString = AsdTWCASHAJ_INTEGRITY_STRING;  // This variable is fixed at gen time
    ...
    RWCString HostLUName (void)
    {
        ...
        AddXMLField(&m_Sess, m_Sess.pIntegrityTag, AsdTWCASHAJ_INTEGRITY_STRING, NULL);
    ...
    long Execute (void)
    {
        ...
        AddXMLField(&m_Sess, m_Sess.pCRCTag, AsdTWCASHAJ_CRC_STRING, NULL);
        AddXMLRecord(&m_Sess, m_Sess.pDataTag, -1);       // DATA
        CTSHeader.Save(&m_Sess);
        CTWCASHIN.Save(&m_Sess);
        CTSMDA.Save(&m_Sess);
        ...
        if ((ret = ExecuteXMLTransaction(&m_Sess)) == 0)
        {
            if (GetXMLRecord(&m_Sess, m_Sess.pAceTag, NULL, NULL))
            {
                ...
                {
                    if(GetXMLRecord(&m_Sess, "TWCASHAJ", NULL, NULL))
                    {
                        CTSHeader.Load(&m_Sess);
                        CTWCASHIN.Load(&m_Sess);
                        CTSMDA.Load(&m_Sess);
    
                        CloseXMLRecord(&m_Sess, "TWCASHAJ");
                    }
    ...
    //**************** Start AsdTWCASHIN Definition ****************
    //                 TELLER WORKSTATION CASH INPUT DAG FOR BUY/SELL CASH, ETC.   
    ...
    class AsdTWCASHIN
    {
      private:
        ...
        bool      m_bExpansionArea;
        char      m_szExpansionArea[20+1];
    
      public:
        long      m_lIndex;

        AsdTWCASHIN ()  // Constructor
        {
            m_bHostInitOverride = false;
            strcpy(m_szHostInitOverride, "");
            m_bReceived = false;
            ...
            m_bExpansionArea = false;
            strcpy(m_szExpansionArea, "");
        }
        ...
        //    EXPANSION AREA
        long lExpansionArea () { return 20; };
        RWCString ExpansionArea (void)
        {
            return m_szExpansionArea;
        }
        long ExpansionArea (const RWCString& rwc)
        {
            return SetString(rwc, m_szExpansionArea, sizeof(m_szExpansionArea), &m_bExpansionArea);
        }
        ...
        bool HasDataToSave (void)
        {
            ...
            if (m_bExpansionArea) return true;
            return false;
        }
        ...
        void Save (SESSIONDATA *pSess)
        {
            if (HasDataToSave())
            {
                AddXMLRecord(pSess, "TWCASHIN", m_lIndex);
                AddXMLField(pSess, "HostInitOverride", m_szHostInitOverride, &m_bHostInitOverride);
                AddXMLField(pSess, "DrawerNumber", m_szDrawerNumber, &m_bDrawerNumber);
                AddXMLField(pSess, "Amount", m_szAmount, &m_bAmount);
                AddXMLField(pSess, "ExpansionArea", m_szExpansionArea, &m_bExpansionArea);
                AddXMLCloseRecord(pSess, "TWCASHIN");  // ACE
            };
        }

        void Load (SESSIONDATA *pSess)
        {
            if (GetXMLRecord(pSess, "TWCASHIN", m_lIndex, &m_bReceived))
            {
                GetXMLField(pSess, "DrawerNumber", m_szDrawerNumber);
                GetXMLField(pSess, "Amount", m_szAmount);
                GetXMLField(pSess, "ExpansionArea", m_szExpansionArea);
                CloseXMLRecord(pSess, "TWCASHIN");
            };
        }
    };

    AsdTWCASHIN CTWCASHIN;
    AsdTWCASHIN *TWCASHIN;
    
    //**************** End AsdTWCASHIN Definition ****************
...
//**************** End AsdTWCASHAJData Definition ****************

typedef AsdTWCASHAJData *LPTWCASHAJ;
typedef AsdTWCASHAJData::AsdTSHeader *LPTWCASHAJ_TSHeader;
typedef AsdTWCASHAJData::AsdTWCASHIN *LPTWCASHAJ_TWCASHIN;
typedef AsdTWCASHAJData::AsdTSMDA *LPTWCASHAJ_TSMDA;
typedef AsdTWCASHAJData::AsdTSMDA::AsdMdaTrmMsg *LPTWCASHAJ_TSMDA_MdaTrmMsg;
typedef AsdTWCASHAJData::AsdTSMDA::AsdMdaTrmMsg::AsdMdaMsg *LPTWCASHAJ_TSMDA_MdaTrmMsg_MdaMsg;
#endif    // AsdTWCASHAJData_H
