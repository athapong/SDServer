#ifndef AsdSDPCAICIData_H
#define AsdSDPCAICIData_H

#include <stdlib.h>
#ifdef  AsdSDPCAICI_INTEGRITY_STRING
#undef  AsdSDPCAICI_INTEGRITY_STRING
#endif
#define AsdSDPCAICI_INTEGRITY_STRING        ""

#ifdef  AsdSDPCAICI_CRC_STRING
#undef  AsdSDPCAICI_CRC_STRING
#endif
#define AsdSDPCAICI_CRC_STRING "3021055808"
...
//**************** Start AsdSDPCAICIData Definition ****************
//                 Als Customer Inquiry

//                 SDPCAICIData.h generated by AceGen2 ver. 2.5.09

class AsdSDPCAICIData
{
  public:
  ...
  public:
    AsdSDPCAICIData ()  // Constructors
    {
        InitializeSessionData(&m_Sess);                      // InitializeSessionData declared in bcAce2.h
        m_Sess.pFileName = "SDPCAICI.ace";                       // This variable is fixed at gen time
        m_Sess.pProtocol = "TLU0";                           // This variable is fixed at gen time
        m_Sess.pIntegrityString = AsdSDPCAICI_INTEGRITY_STRING;  // This variable is fixed at gen time
    ...
    RWCString HostLUName (void)
    {
        ...
        AddXMLField(&m_Sess, m_Sess.pIntegrityTag, AsdSDPCAICI_INTEGRITY_STRING, NULL);
    ...
    long Execute (void)
    {
        ...
        if ((ret = ExecuteXMLTransaction(&m_Sess)) == 0)
        {
            if (GetXMLRecord(&m_Sess, m_Sess.pAceTag, NULL, NULL))
            {
                ...
                {
                    if(GetXMLRecord(&m_Sess, "SDPCAICI", NULL, NULL))
                    {
                        CTSHeader.Load(&m_Sess);
                        CSDAMCMII.Load(&m_Sess);
                        CSDAMCRIO.Load(&m_Sess);
                        CTSMDA.Load(&m_Sess);
    
                        CloseXMLRecord(&m_Sess, "SDPCAICI");
                    }
    ...
    //**************** Start AsdSDAMCRIO Definition ****************
    //                 SD ALS CUSTOMER RELATIONSHIPS                               
    
    //    SDAMCRIO.map
    //    generated by Dsm2Ace ver. 2.3.03
    //    on 02/25/03 09:14:27
    class AsdSDAMCRIO
    {
      private:
        bool      m_bHostInitOverride;
      ...
      public:
        ...
        //    CUSTOMER RELATIONSHIP HOLD KEY.
        long lCfRlHoldKey () { return 27; };
        RWCString CfRlHoldKey (void)
        {
            return m_szCfRlHoldKey;
        }
        long CfRlHoldKey (const RWCString& rwc)
        {
            return SetString(rwc, m_szCfRlHoldKey, sizeof(m_szCfRlHoldKey), &m_bCfRlHoldKey);
        }
        ...
        //    NUMBER OF CUSTOMER RELATIONSHIP RECORDS IN THE DAG.
        long lCfRlRecCnt () { return 4; };
        RWCString CfRlRecCnt (void)
        {
            return AceZeroSupp(m_szCfRlRecCnt, &m_szSuppressBuff[0]);
        }
        RWCString CfRlRecCntRaw (void)
        {
            return m_szCfRlRecCnt;
        }
        long CfRlRecCnt (const RWCString& rwc)
        {
            return SetString(rwc, m_szCfRlRecCnt, sizeof(m_szCfRlRecCnt), &m_bCfRlRecCnt);
        }
        
        bool HasDataToSave (void)
        {
            if (m_bHostInitOverride) return true;
            if (m_bCfRlHoldKey) return true;
            if (m_bMoreRecordsPresent) return true;
            if (m_bKeyedReadFlag) return true;
            if (m_bCfRlRecCnt) return true;
            long lOccurs;
            for (lOccurs = 0; lOccurs < 16; lOccurs++)
            {
                if(CCfRelData[lOccurs].HasDataToSave())
                {
                    return true;
                }
            }
        ...
        void Save (SESSIONDATA *pSess)
        {
            if (HasDataToSave())
            {
                AddXMLRecord(pSess, "SDAMCRIO", m_lIndex);
                AddXMLField(pSess, "HostInitOverride", m_szHostInitOverride, &m_bHostInitOverride);
                AddXMLField(pSess, "CfRlHoldKey", m_szCfRlHoldKey, &m_bCfRlHoldKey);
                ...
                AddXMLField(pSess, "CfRlRecCnt", m_szCfRlRecCnt, &m_bCfRlRecCnt);
                long lOccurs;
                for (lOccurs = 0; lOccurs < 16; lOccurs++)
                {
                    CCfRelData[lOccurs].Save(pSess);
                }
                AddXMLCloseRecord(pSess, "SDAMCRIO");  // ACE
            };
        }

        void Load (SESSIONDATA *pSess)
        {
            if (GetXMLRecord(pSess, "SDAMCRIO", m_lIndex, &m_bReceived))
            {
                GetXMLField(pSess, "CfRlHoldKey", m_szCfRlHoldKey);
                GetXMLField(pSess, "MoreRecordsPresent", m_szMoreRecordsPresent);
                GetXMLField(pSess, "KeyedReadFlag", m_szKeyedReadFlag);
                GetXMLField(pSess, "CfRlRecCnt", m_szCfRlRecCnt);
                long lOccurs;
                for (lOccurs = 0; lOccurs < 16; lOccurs++)
                {
                    CCfRelData[lOccurs].Load(pSess);
                }
                CloseXMLRecord(pSess, "SDAMCRIO");
            };
        }
        
        //**************** Start AsdCfRelData Definition ****************
        ...
        class AsdCfRelData
        {
          private:
            ...
            bool      m_bCfLiabilityType;
            char      m_szCfLiabilityType[1+1];
            bool      m_bCfRelTypeDesc;
            char      m_szCfRelTypeDesc[10+1];
            bool      m_bCfShortName;
            char      m_szCfShortName[10+1];
        
          public:
            long      m_lIndex;

            AsdCfRelData ()  // Constructor
            {
                m_bReceived = false;
                m_lIndex = -1;
                m_bRtCtl1 = false;
                ...
                m_bCfLiabilityType = false;
                strcpy(m_szCfLiabilityType, "");
                m_bCfRelTypeDesc = false;
                strcpy(m_szCfRelTypeDesc, "");
                m_bCfShortName = false;
                strcpy(m_szCfShortName, "");
            ...
            //    INDICATES CONTRACTUAL LIABILITY FOR THIS ACCOUNT.
            long lCfLiabilityType () { return 1; };
            RWCString CfLiabilityType (void)
            {
                return m_szCfLiabilityType;
            }
            long CfLiabilityType (const RWCString& rwc)
            {
                return SetString(rwc, m_szCfLiabilityType, sizeof(m_szCfLiabilityType), &m_bCfLiabilityType);
            }
            
            //    CUST REL TYPE DESC
            long lCfRelTypeDesc () { return 10; };
            RWCString CfRelTypeDesc (void)
            {
                return m_szCfRelTypeDesc;
            }
            long CfRelTypeDesc (const RWCString& rwc)
            {
                return SetString(rwc, m_szCfRelTypeDesc, sizeof(m_szCfRelTypeDesc), &m_bCfRelTypeDesc);
            }
            
            //    CUST REL SHORT NAME
            long lCfShortName () { return 10; };
            RWCString CfShortName (void)
            {
                return m_szCfShortName;
            }
            ...
            bool HasDataToSave (void)
            {
                ...
                if (m_bCfLiabilityType) return true;
                if (m_bCfRelTypeDesc) return true;
                if (m_bCfShortName) return true;
                return false;
            }
            ...
            void Save (SESSIONDATA *pSess)
            {
                if (HasDataToSave())
                {
                    AddXMLRecord(pSess, "CfRelData", m_lIndex);
                    ...
                    AddXMLField(pSess, "CfLiabilityType", m_szCfLiabilityType, &m_bCfLiabilityType);
                    AddXMLField(pSess, "CfRelTypeDesc", m_szCfRelTypeDesc, &m_bCfRelTypeDesc);
                    AddXMLField(pSess, "CfShortName", m_szCfShortName, &m_bCfShortName);
                    AddXMLCloseRecord(pSess, "CfRelData");  // ACE
                };
            }

            void Load (SESSIONDATA *pSess)
            {
                if (GetXMLRecord(pSess, "CfRelData", m_lIndex, &m_bReceived))
                {
                    ...
                    GetXMLField(pSess, "CfLiabilityType", m_szCfLiabilityType);
                    GetXMLField(pSess, "CfRelTypeDesc", m_szCfRelTypeDesc);
                    GetXMLField(pSess, "CfShortName", m_szCfShortName);
                    CloseXMLRecord(pSess, "CfRelData");
                };
            }
        };

        AsdCfRelData  CCfRelData[16];
        AsdCfRelData *CfRelData[16];
        
        //**************** End AsdCfRelData Definition ****************
        
    };

    AsdSDAMCRIO CSDAMCRIO;
    AsdSDAMCRIO *SDAMCRIO;
    
    //**************** End AsdSDAMCRIO Definition ****************
...
//**************** End AsdSDPCAICIData Definition ****************

typedef AsdSDPCAICIData *LPSDPCAICI;
typedef AsdSDPCAICIData::AsdTSHeader *LPSDPCAICI_TSHeader;
typedef AsdSDPCAICIData::AsdSDAMCMII *LPSDPCAICI_SDAMCMII;
typedef AsdSDPCAICIData::AsdSDAMCMII::AsdCfLineInfo *LPSDPCAICI_SDAMCMII_CfLineInfo;
typedef AsdSDPCAICIData::AsdSDAMCRIO *LPSDPCAICI_SDAMCRIO;
typedef AsdSDPCAICIData::AsdSDAMCRIO::AsdCfRelData *LPSDPCAICI_SDAMCRIO_CfRelData;
typedef AsdSDPCAICIData::AsdTSMDA *LPSDPCAICI_TSMDA;
typedef AsdSDPCAICIData::AsdTSMDA::AsdMdaTrmMsg *LPSDPCAICI_TSMDA_MdaTrmMsg;
typedef AsdSDPCAICIData::AsdTSMDA::AsdMdaTrmMsg::AsdMdaMsg *LPSDPCAICI_TSMDA_MdaTrmMsg_MdaMsg;
#endif    // AsdSDPCAICIData_H
