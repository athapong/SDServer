#ifndef AsdTPVSEData_H
#define AsdTPVSEData_H

#include <stdlib.h>
#ifdef  AsdTPVSE_INTEGRITY_STRING
#undef  AsdTPVSE_INTEGRITY_STRING
#endif
#define AsdTPVSE_INTEGRITY_STRING        ""

#ifdef  AsdTPVSE_CRC_STRING
#undef  AsdTPVSE_CRC_STRING
#endif
#define AsdTPVSE_CRC_STRING "3390246657"
...
//**************** Start AsdTPVSEData Definition ****************
//                 Third Party / VSE

//                 TPVSEData.h generated by AceGen2 ver. 2.5.09
...
class AsdTPVSEData
{
  ...
  public:
    AsdTPVSEData ()  // Constructors
    {
        InitializeSessionData(&m_Sess);                      // InitializeSessionData declared in bcAce2.h
        m_Sess.pFileName = "TPVSE.ace";                       // This variable is fixed at gen time
        m_Sess.pProtocol = "TLU0";                           // This variable is fixed at gen time
        m_Sess.pIntegrityString = AsdTPVSE_INTEGRITY_STRING;  // This variable is fixed at gen time
    ...
    RWCString HostLUName (void)
    {
        ...
        AddXMLField(&m_Sess, m_Sess.pIntegrityTag, AsdTPVSE_INTEGRITY_STRING, NULL);
    ...
    long Execute (void)
    {
        ...
        AddXMLField(&m_Sess, m_Sess.pCRCTag, AsdTPVSE_CRC_STRING, NULL);
        AddXMLRecord(&m_Sess, m_Sess.pDataTag, -1);       // DATA
        CTSHeader.Save(&m_Sess);
        CVSETRAN.Save(&m_Sess);
        CTSMDA.Save(&m_Sess);
        ...
        if ((ret = ExecuteXMLTransaction(&m_Sess)) == 0)
        {
            if (GetXMLRecord(&m_Sess, m_Sess.pAceTag, NULL, NULL))
            {
                ...
                {
                    if(GetXMLRecord(&m_Sess, "TPVSE", NULL, NULL))
                    {
                        CTSHeader.Load(&m_Sess);
                        CVSETRAN.Load(&m_Sess);
                        CTSMDA.Load(&m_Sess);
    
                        CloseXMLRecord(&m_Sess, "TPVSE");
                    }
    ...
    class AsdVSETRAN
    {
      private:
        ...
        bool      m_bTranTime;
        long      m_lTranTime;
        bool      m_bWorkstationID;
        short     m_nWorkstationID;
        bool      m_bSafeCode;
        short     m_nSafeCode;
        bool      m_bTellerID;
        short     m_nTellerID;
        bool      m_bSupervisorID;
        short     m_nSupervisorID;
        bool      m_bSupervisorLevel;
        short     m_nSupervisorLevel;
        bool      m_bSafeFlag;
        short     m_nSafeFlag;
        bool      m_bInputData;
        char      m_szInputData[500+1];
        bool      m_bReplyData[50];
        char      m_szReplyData[50][100+1];
    
      public:
        long      m_lIndex;

        AsdVSETRAN ()  // Constructor
        {
            m_bHostInitOverride = false;
            strcpy(m_szHostInitOverride, "");
            m_bReceived = false;
            ...
            m_bTranTime = false;
            m_lTranTime = 0000;
            m_bWorkstationID = false;
            m_nWorkstationID = 0;
            m_bSafeCode = false;
            m_nSafeCode = 00;
            m_bTellerID = false;
            m_nTellerID = 00;
            m_bSupervisorID = false;
            m_nSupervisorID = 00;
            m_bSupervisorLevel = false;
            m_nSupervisorLevel = 0;
            m_bSafeFlag = false;
            m_nSafeFlag = 0;
            m_bInputData = false;
            strcpy(m_szInputData, "");
            long lOccurs;
            for (lOccurs = 0; lOccurs < 50; lOccurs++)
            {
                m_bReplyData[lOccurs] = false;
                strcpy(m_szReplyData[lOccurs], "");
            }
        }
        ...
        //    WS-VSE-CONTROL-ID
        short ControlID (void)
        {
            return m_nControlID;
        }
        long ControlID (const short nValue)
        {
            return SetShort(nValue, &m_nControlID, &m_bControlID);
        }
        ...
        long TranTime (long lValue)
        {
            return SetLong(lValue, &m_lTranTime, &m_bTranTime);
        }
        
        //    WS-VSE-WORK-STATION
        short WorkstationID (void)
        {
            return m_nWorkstationID;
        }
        long WorkstationID (const short nValue)
        {
            return SetShort(nValue, &m_nWorkstationID, &m_bWorkstationID);
        }
        ...
        long SafeCode (const short nValue)
        {
            return SetShort(nValue, &m_nSafeCode, &m_bSafeCode);
        }
        ...
        long TellerID (const short nValue)
        {
            return SetShort(nValue, &m_nTellerID, &m_bTellerID);
        }
        
        //    WS-VSE-SPRVSR-ID
        short SupervisorID (void)
        {
            return m_nSupervisorID;
        }
        long SupervisorID (const short nValue)
        {
            return SetShort(nValue, &m_nSupervisorID, &m_bSupervisorID);
        }
        
        //    WS-VSE-SPRVSR-LVL
        short SupervisorLevel (void)
        {
            return m_nSupervisorLevel;
        }
        long SupervisorLevel (const short nValue)
        {
            return SetShort(nValue, &m_nSupervisorLevel, &m_bSupervisorLevel);
        }
        ...
        long SafeFlag (const short nValue)
        {
            return SetShort(nValue, &m_nSafeFlag, &m_bSafeFlag);
        }
        
        //    WS-VSE-DATA
        long lInputData () { return 500; };
        RWCString InputData (void)
        {
            return m_szInputData;
        }
        long InputData (const RWCString& rwc)
        {
            return SetString(rwc, m_szInputData, sizeof(m_szInputData), &m_bInputData);
        }
        
        //    WS-VSE-REPLY
        long lReplyData () { return 100; };
        RWCString ReplyData (long i)
        {
            if (i >= 0 && i < 50)
                return m_szReplyData[i];
            else
                return "";
        }
        long ReplyData (const RWCString& rwc, long i)
        {
            if (i >= 0 && i < 50)
                return SetString(rwc, m_szReplyData[i], sizeof(m_szReplyData[i]), &m_bReplyData[i]);
            else
                return -1;
        }
        ...
        bool HasDataToSave (void)
        {
            ...
            if (m_bTranTime) return true;
            if (m_bWorkstationID) return true;
            if (m_bSafeCode) return true;
            if (m_bTellerID) return true;
            if (m_bSupervisorID) return true;
            if (m_bSupervisorLevel) return true;
            if (m_bSafeFlag) return true;
            if (m_bInputData) return true;
            long lOccurs;
            for (lOccurs = 0; lOccurs < 50; lOccurs++)
                if (m_bReplyData[lOccurs]) return true;
            return false;
        }
        ...
        void Save (SESSIONDATA *pSess)
        {
            if (HasDataToSave())
            {
                AddXMLRecord(pSess, "VSETRAN", m_lIndex);
                AddXMLField(pSess, "HostInitOverride", m_szHostInitOverride, &m_bHostInitOverride);
                AddXMLField(pSess, "TranCode", m_szTranCode, &m_bTranCode);
                AddXMLField(pSess, "ControlID", m_nControlID, &m_bControlID);
                AddXMLField(pSess, "TranTime", m_lTranTime, &m_bTranTime);
                AddXMLField(pSess, "WorkstationID", m_nWorkstationID, &m_bWorkstationID);
                AddXMLField(pSess, "SafeCode", m_nSafeCode, &m_bSafeCode);
                AddXMLField(pSess, "TellerID", m_nTellerID, &m_bTellerID);
                AddXMLField(pSess, "SupervisorID", m_nSupervisorID, &m_bSupervisorID);
                AddXMLField(pSess, "SupervisorLevel", m_nSupervisorLevel, &m_bSupervisorLevel);
                AddXMLField(pSess, "SafeFlag", m_nSafeFlag, &m_bSafeFlag);
                AddXMLField(pSess, "InputData", m_szInputData, &m_bInputData);
                long lOccurs;
                for (lOccurs = 0; lOccurs < 50; lOccurs++)
                    AddXMLField(pSess, "ReplyData", m_szReplyData[lOccurs], &m_bReplyData[lOccurs], lOccurs);
                AddXMLCloseRecord(pSess, "VSETRAN");  // ACE
            };
        }

        void Load (SESSIONDATA *pSess)
        {
            if (GetXMLRecord(pSess, "VSETRAN", m_lIndex, &m_bReceived))
            {
                GetXMLField(pSess, "TranCode", m_szTranCode);
                GetXMLField(pSess, "ControlID", &m_nControlID);
                GetXMLField(pSess, "TranTime", &m_lTranTime);
                GetXMLField(pSess, "WorkstationID", &m_nWorkstationID);
                GetXMLField(pSess, "SafeCode", &m_nSafeCode);
                GetXMLField(pSess, "TellerID", &m_nTellerID);
                GetXMLField(pSess, "SupervisorID", &m_nSupervisorID);
                GetXMLField(pSess, "SupervisorLevel", &m_nSupervisorLevel);
                GetXMLField(pSess, "SafeFlag", &m_nSafeFlag);
                GetXMLField(pSess, "InputData", m_szInputData);
                long lOccurs;
                for (lOccurs = 0; lOccurs < 50; lOccurs++)
                    GetXMLField(pSess, "ReplyData", m_szReplyData[lOccurs], lOccurs);
                CloseXMLRecord(pSess, "VSETRAN");
            };
        }
    };
...
//**************** End AsdTPVSEData Definition ****************

typedef AsdTPVSEData *LPTPVSE;
typedef AsdTPVSEData::AsdTSHeader *LPTPVSE_TSHeader;
typedef AsdTPVSEData::AsdVSETRAN *LPTPVSE_VSETRAN;
typedef AsdTPVSEData::AsdTSMDA *LPTPVSE_TSMDA;
typedef AsdTPVSEData::AsdTSMDA::AsdMdaTrmMsg *LPTPVSE_TSMDA_MdaTrmMsg;
typedef AsdTPVSEData::AsdTSMDA::AsdMdaTrmMsg::AsdMdaMsg *LPTPVSE_TSMDA_MdaTrmMsg_MdaMsg;
#endif    // AsdTPVSEData_H
