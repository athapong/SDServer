#ifndef AsdSDPCIMODData_H
#define AsdSDPCIMODData_H

#include <stdlib.h>
#ifdef  AsdSDPCIMOD_INTEGRITY_STRING
#undef  AsdSDPCIMOD_INTEGRITY_STRING
#endif
#define AsdSDPCIMOD_INTEGRITY_STRING        ""

#ifdef  AsdSDPCIMOD_CRC_STRING
#undef  AsdSDPCIMOD_CRC_STRING
#endif
#define AsdSDPCIMOD_CRC_STRING "3285142301"
...
//**************** Start AsdSDPCIMODData Definition ****************
//                 Secure Funding Inquiry

//                 SDPCIMODData.h generated by AceGen2 ver. 2.5.09

class AsdSDPCIMODData
{
  public:
  ...
  public:
    AsdSDPCIMODData ()  // Constructors
    {
        InitializeSessionData(&m_Sess);                      // InitializeSessionData declared in bcAce2.h
        m_Sess.pFileName = "SDPCIMOD.ace";                       // This variable is fixed at gen time
        m_Sess.pProtocol = "TLU0";                           // This variable is fixed at gen time
        m_Sess.pIntegrityString = AsdSDPCIMOD_INTEGRITY_STRING;  // This variable is fixed at gen time
    ...
    RWCString HostLUName (void)
    {
        ...
        AddXMLField(&m_Sess, m_Sess.pIntegrityTag, AsdSDPCIMOD_INTEGRITY_STRING, NULL);
    ...
    long Execute (void)
    {
        ...
        AddXMLField(&m_Sess, m_Sess.pCRCTag, AsdSDPCIMOD_CRC_STRING, NULL);
        AddXMLRecord(&m_Sess, m_Sess.pDataTag, -1);       // DATA
        CTSHeader.Save(&m_Sess);
        CSDIMODP.Save(&m_Sess);
        CTSMDA.Save(&m_Sess);
        ...
        if ((ret = ExecuteXMLTransaction(&m_Sess)) == 0)
        {
            if (GetXMLRecord(&m_Sess, m_Sess.pAceTag, NULL, NULL))
            {
                ...
                {
                    if(GetXMLRecord(&m_Sess, "SDPCIMOD", NULL, NULL))
                    {
                        CTSHeader.Load(&m_Sess);
                        CSDIMODP.Load(&m_Sess);
                        CTSMDA.Load(&m_Sess);
    
                        CloseXMLRecord(&m_Sess, "SDPCIMOD");
                    }
    ...
    //**************** Start AsdSDIMODP Definition ****************
    //                 SDIMODP                                                     
    
    //    SDIMODP.map
    //    generated by Dsm2Ace ver. 2.3.03
    //    on 02/19/03 14:33:13
    class AsdSDIMODP
    {
      private:
        bool      m_bHostInitOverride;
        ...
        bool      m_bNoStAccts;
        char      m_szNoStAccts[4+1];
        bool      m_bTotCollateralAmt;
        char      m_szTotCollateralAmt[19+1];
    
      public:
        long      m_lIndex;

        AsdSDIMODP ()  // Constructor
        {
            m_bHostInitOverride = false;
            strcpy(m_szHostInitOverride, "");
            m_bReceived = false;
            ...
            m_bNoStAccts = false;
            strcpy(m_szNoStAccts, "+000");
            m_bTotCollateralAmt = false;
            strcpy(m_szTotCollateralAmt, "+000000000000000.00");
            long lOccurs;
        
            for (lOccurs = 0; lOccurs < 30; lOccurs++)
            {
                StCollateralAccts[lOccurs]  = &CStCollateralAccts[lOccurs];
                CStCollateralAccts[lOccurs].m_lIndex = lOccurs;
            }
        }
        ...
        //    NO-ST-ACCTS
        long lNoStAccts () { return 4; };
        RWCString NoStAccts (void)
        {
            return AceZeroSupp(m_szNoStAccts, &m_szSuppressBuff[0]);
        }
        RWCString NoStAcctsRaw (void)
        {
            return m_szNoStAccts;
        }
        long NoStAccts (const RWCString& rwc)
        {
            return SetString(rwc, m_szNoStAccts, sizeof(m_szNoStAccts), &m_bNoStAccts);
        }
        
        //    TOT-COLLATERAL-AMT
        long lTotCollateralAmt () { return 19; };
        RWCString TotCollateralAmt (void)
        {
            return AceZeroSupp(m_szTotCollateralAmt, &m_szSuppressBuff[0]);
        }
        RWCString TotCollateralAmtRaw (void)
        {
            return m_szTotCollateralAmt;
        }
        long TotCollateralAmt (const RWCString& rwc)
        {
            return SetString(rwc, m_szTotCollateralAmt, sizeof(m_szTotCollateralAmt), &m_bTotCollateralAmt);
        }
        ...
        bool HasDataToSave (void)
        {
            ...
            if (m_bNoStAccts) return true;
            if (m_bTotCollateralAmt) return true;
            long lOccurs;
            for (lOccurs = 0; lOccurs < 30; lOccurs++)
            {
                if(CStCollateralAccts[lOccurs].HasDataToSave())
                {
                    return true;
                }
            }
        ...
        void Save (SESSIONDATA *pSess)
        {
            if (HasDataToSave())
            {
                AddXMLRecord(pSess, "SDIMODP", m_lIndex);
                AddXMLField(pSess, "HostInitOverride", m_szHostInitOverride, &m_bHostInitOverride);
                ...
                AddXMLField(pSess, "NoStAccts", m_szNoStAccts, &m_bNoStAccts);
                AddXMLField(pSess, "TotCollateralAmt", m_szTotCollateralAmt, &m_bTotCollateralAmt);
                long lOccurs;
                for (lOccurs = 0; lOccurs < 30; lOccurs++)
                {
                    CStCollateralAccts[lOccurs].Save(pSess);
                }
                AddXMLCloseRecord(pSess, "SDIMODP");  // ACE
            };
        }

        void Load (SESSIONDATA *pSess)
        {
            if (GetXMLRecord(pSess, "SDIMODP", m_lIndex, &m_bReceived))
            {
                ...
                GetXMLField(pSess, "NoStAccts", m_szNoStAccts);
                GetXMLField(pSess, "TotCollateralAmt", m_szTotCollateralAmt);
                long lOccurs;
                for (lOccurs = 0; lOccurs < 30; lOccurs++)
                {
                    CStCollateralAccts[lOccurs].Load(pSess);
                }
                CloseXMLRecord(pSess, "SDIMODP");
            };
        }
        
        //**************** Start AsdStCollateralAccts Definition ****************
        //                 ST COLLATERAL ACCTS
        
        class AsdStCollateralAccts
        {
          private:
            bool      m_bReceived;
            char      m_szSuppressBuff[100];
            ...
            bool      m_bCollateralAmt;
            char      m_szCollateralAmt[17+1];
            bool      m_bCdRate;
            char      m_szCdRate[11+1];
            bool      m_bCdEffectiveDate;
            char      m_szCdEffectiveDate[8+1];
            bool      m_bCdMaturityDate;
            char      m_szCdMaturityDate[8+1];
        
          public:
            long      m_lIndex;

            AsdStCollateralAccts ()  // Constructor
            {
                m_bReceived = false;
                m_lIndex = -1;
                m_bStControl1 = false;
                ...
                m_bCollateralAmt = false;
                strcpy(m_szCollateralAmt, "+0000000000000.00");
                m_bCdRate = false;
                strcpy(m_szCdRate, "+0.00000000");
                m_bCdEffectiveDate = false;
                strcpy(m_szCdEffectiveDate, "");
                m_bCdMaturityDate = false;
                strcpy(m_szCdMaturityDate, "");
            ...
            //    COLLATERAL-AMT
            long lCollateralAmt () { return 17; };
            RWCString CollateralAmt (void)
            {
                return AceZeroSupp(m_szCollateralAmt, &m_szSuppressBuff[0]);
            }
            RWCString CollateralAmtRaw (void)
            {
                return m_szCollateralAmt;
            }
            long CollateralAmt (const RWCString& rwc)
            {
                return SetString(rwc, m_szCollateralAmt, sizeof(m_szCollateralAmt), &m_bCollateralAmt);
            }
            
            //    CD-RATE
            long lCdRate () { return 11; };
            RWCString CdRate (void)
            {
                return AceZeroSupp(m_szCdRate, &m_szSuppressBuff[0]);
            }
            RWCString CdRateRaw (void)
            {
                return m_szCdRate;
            }
            long CdRate (const RWCString& rwc)
            {
                return SetString(rwc, m_szCdRate, sizeof(m_szCdRate), &m_bCdRate);
            }
            
            //    CD-EFFECTIVE-DATE
            long lCdEffectiveDate () { return 8; };
            RWCString CdEffectiveDate (void)
            {
                return m_szCdEffectiveDate;
            }
            long CdEffectiveDate (const RWCString& rwc)
            {
                return SetString(rwc, m_szCdEffectiveDate, sizeof(m_szCdEffectiveDate), &m_bCdEffectiveDate);
            }
            
            //    CD-MATURITY-DATE
            long lCdMaturityDate () { return 8; };
            RWCString CdMaturityDate (void)
            {
                return m_szCdMaturityDate;
            }
            long CdMaturityDate (const RWCString& rwc)
            {
                return SetString(rwc, m_szCdMaturityDate, sizeof(m_szCdMaturityDate), &m_bCdMaturityDate);
            }
            ...
            bool HasDataToSave (void)
            {
                ...
                if (m_bCollateralAmt) return true;
                if (m_bCdRate) return true;
                if (m_bCdEffectiveDate) return true;
                if (m_bCdMaturityDate) return true;
                return false;
            }
            ...
            void Save (SESSIONDATA *pSess)
            {
                if (HasDataToSave())
                {
                    AddXMLRecord(pSess, "StCollateralAccts", m_lIndex);
                    ...
                    AddXMLField(pSess, "CollateralAmt", m_szCollateralAmt, &m_bCollateralAmt);
                    AddXMLField(pSess, "CdRate", m_szCdRate, &m_bCdRate);
                    AddXMLField(pSess, "CdEffectiveDate", m_szCdEffectiveDate, &m_bCdEffectiveDate);
                    AddXMLField(pSess, "CdMaturityDate", m_szCdMaturityDate, &m_bCdMaturityDate);
                    AddXMLCloseRecord(pSess, "StCollateralAccts");  // ACE
                };
            }

            void Load (SESSIONDATA *pSess)
            {
                if (GetXMLRecord(pSess, "StCollateralAccts", m_lIndex, &m_bReceived))
                {
                    ...
                    GetXMLField(pSess, "CollateralAmt", m_szCollateralAmt);
                    GetXMLField(pSess, "CdRate", m_szCdRate);
                    GetXMLField(pSess, "CdEffectiveDate", m_szCdEffectiveDate);
                    GetXMLField(pSess, "CdMaturityDate", m_szCdMaturityDate);
                    CloseXMLRecord(pSess, "StCollateralAccts");
                };
            }
        };

        AsdStCollateralAccts  CStCollateralAccts[30];
        AsdStCollateralAccts *StCollateralAccts[30];
        
        //**************** End AsdStCollateralAccts Definition ****************
...
//**************** End AsdSDPCIMODData Definition ****************

typedef AsdSDPCIMODData *LPSDPCIMOD;
typedef AsdSDPCIMODData::AsdTSHeader *LPSDPCIMOD_TSHeader;
typedef AsdSDPCIMODData::AsdSDIMODP *LPSDPCIMOD_SDIMODP;
typedef AsdSDPCIMODData::AsdSDIMODP::AsdStCollateralAccts *LPSDPCIMOD_SDIMODP_StCollateralAccts;
typedef AsdSDPCIMODData::AsdTSMDA *LPSDPCIMOD_TSMDA;
typedef AsdSDPCIMODData::AsdTSMDA::AsdMdaTrmMsg *LPSDPCIMOD_TSMDA_MdaTrmMsg;
typedef AsdSDPCIMODData::AsdTSMDA::AsdMdaTrmMsg::AsdMdaMsg *LPSDPCIMOD_TSMDA_MdaTrmMsg_MdaMsg;
#endif    // AsdSDPCIMODData_H
