#ifndef AsdSDPCAMAFData_H
#define AsdSDPCAMAFData_H

#include <stdlib.h>
#ifdef  AsdSDPCAMAF_INTEGRITY_STRING
#undef  AsdSDPCAMAF_INTEGRITY_STRING
#endif
#define AsdSDPCAMAF_INTEGRITY_STRING        ""

#ifdef  AsdSDPCAMAF_CRC_STRING
#undef  AsdSDPCAMAF_CRC_STRING
#endif
#define AsdSDPCAMAF_CRC_STRING "918365984"
...
//**************** Start AsdSDPCAMAFData Definition ****************
//                 Automatic Payment

//                 SDPCAMAFData.h generated by AceGen2 ver. 2.5.09

class AsdSDPCAMAFData
{
  public:
  ...
  public:
    AsdSDPCAMAFData ()  // Constructors
    {
        InitializeSessionData(&m_Sess);                      // InitializeSessionData declared in bcAce2.h
        m_Sess.pFileName = "SDPCAMAF.ace";                       // This variable is fixed at gen time
        m_Sess.pProtocol = "TLU0";                           // This variable is fixed at gen time
        m_Sess.pIntegrityString = AsdSDPCAMAF_INTEGRITY_STRING;  // This variable is fixed at gen time
    ...
    RWCString HostLUName (void)
    {
        ...
        AddXMLField(&m_Sess, m_Sess.pIntegrityTag, AsdSDPCAMAF_INTEGRITY_STRING, NULL);
    ...
    long Execute (void)
    {
        ...
        AddXMLField(&m_Sess, m_Sess.pCRCTag, AsdSDPCAMAF_CRC_STRING, NULL);
        AddXMLRecord(&m_Sess, m_Sess.pDataTag, -1);       // DATA
        CTSHeader.Save(&m_Sess);
        CSDAMAFII.Save(&m_Sess);
        CTSMDA.Save(&m_Sess);
        ...
        if ((ret = ExecuteXMLTransaction(&m_Sess)) == 0)
        {
            if (GetXMLRecord(&m_Sess, m_Sess.pAceTag, NULL, NULL))
            {
                ...
                {
                    if(GetXMLRecord(&m_Sess, "SDPCAMAF", NULL, NULL))
                    {
                        CTSHeader.Load(&m_Sess);
                        CSDAMAFII.Load(&m_Sess);
                        CTSMDA.Load(&m_Sess);
    
                        CloseXMLRecord(&m_Sess, "SDPCAMAF");
                    }
    ...
    //**************** Start AsdSDAMAFII Definition ****************
    //                 SD ALS AFFILIATE ACCOUNT INPUT                              
    ...
    class AsdSDAMAFII
    {
      private:
        ...
        bool      m_bBdAutoMeth;
        char      m_szBdAutoMeth[1+1];
        bool      m_bBlLeadDays;
        char      m_szBlLeadDays[4+1];
        bool      m_bAcType;
        char      m_szAcType[3+1];
        bool      m_bAcCd;
        char      m_szAcCd[2+1];
        bool      m_bAcNewRate;
        char      m_szAcNewRate[13+1];
        bool      m_bTranInd;
        char      m_szTranInd[1+1];
    
      public:
        long      m_lIndex;

        AsdSDAMAFII ()  // Constructor
        {
            m_bHostInitOverride = false;
            strcpy(m_szHostInitOverride, "");
            m_bReceived = false;
            ...
            m_bBlLeadDays = false;
            strcpy(m_szBlLeadDays, "+000");
            m_bAcType = false;
            strcpy(m_szAcType, "");
            m_bAcCd = false;
            strcpy(m_szAcCd, "00");
            m_bAcNewRate = false;
            strcpy(m_szAcNewRate, "+0000.0000000");
            m_bTranInd = false;
            strcpy(m_szTranInd, "");
        }
        ...
        //    INDICATES DAYS BEFORE DUE DATE AUTO TRANS IS SENT
        ...
        //    PROCESSING TO BE DONE TO PRODUCE THE DESIRED AUTO DB.
        long lBdAutoMeth () { return 1; };
        ...
        //    NUMBER OF DAYS BEFORE DUE DATE BILLING IS DONE.
        long lBlLeadDays () { return 4; };
        RWCString BlLeadDays (void)
        {
            return AceZeroSupp(m_szBlLeadDays, &m_szSuppressBuff[0]);
        }
        RWCString BlLeadDaysRaw (void)
        {
            return m_szBlLeadDays;
        }
        long BlLeadDays (const RWCString& rwc)
        {
            return SetString(rwc, m_szBlLeadDays, sizeof(m_szBlLeadDays), &m_bBlLeadDays);
        }
        ...
        //    NEW ACCRUAL RATE
        long lAcNewRate () { return 13; };
        RWCString AcNewRate (void)
        {
            return AceZeroSupp(m_szAcNewRate, &m_szSuppressBuff[0]);
        }
        RWCString AcNewRateRaw (void)
        {
            return m_szAcNewRate;
        }
        long AcNewRate (const RWCString& rwc)
        {
            return SetString(rwc, m_szAcNewRate, sizeof(m_szAcNewRate), &m_bAcNewRate);
        }
        
        //    INDICATES WHICH ALS FUNCTION IS TO BE PROCESSED.
        long lTranInd () { return 1; };
        RWCString TranInd (void)
        {
            return m_szTranInd;
        }
        long TranInd (const RWCString& rwc)
        {
            return SetString(rwc, m_szTranInd, sizeof(m_szTranInd), &m_bTranInd);
        }
        ...
        bool HasDataToSave (void)
        {
            ...
            if (m_bBlLeadDays) return true;
            if (m_bAcType) return true;
            if (m_bAcCd) return true;
            if (m_bAcNewRate) return true;
            if (m_bTranInd) return true;
            return false;
        }
        ...
        void Save (SESSIONDATA *pSess)
        {
            if (HasDataToSave())
            {
                AddXMLRecord(pSess, "SDAMAFII", m_lIndex);
                AddXMLField(pSess, "HostInitOverride", m_szHostInitOverride, &m_bHostInitOverride);
                ...
                AddXMLField(pSess, "BlLeadDays", m_szBlLeadDays, &m_bBlLeadDays);
                AddXMLField(pSess, "AcType", m_szAcType, &m_bAcType);
                AddXMLField(pSess, "AcCd", m_szAcCd, &m_bAcCd);
                AddXMLField(pSess, "AcNewRate", m_szAcNewRate, &m_bAcNewRate);
                AddXMLField(pSess, "TranInd", m_szTranInd, &m_bTranInd);
                AddXMLCloseRecord(pSess, "SDAMAFII");  // ACE
            };
        }

        void Load (SESSIONDATA *pSess)
        {
            if (GetXMLRecord(pSess, "SDAMAFII", m_lIndex, &m_bReceived))
            {
                ...
                GetXMLField(pSess, "BlLeadDays", m_szBlLeadDays);
                GetXMLField(pSess, "AcType", m_szAcType);
                GetXMLField(pSess, "AcCd", m_szAcCd);
                GetXMLField(pSess, "AcNewRate", m_szAcNewRate);
                GetXMLField(pSess, "TranInd", m_szTranInd);
                CloseXMLRecord(pSess, "SDAMAFII");
            };
        }
    };

    AsdSDAMAFII CSDAMAFII;
    AsdSDAMAFII *SDAMAFII;
    
    //**************** End AsdSDAMAFII Definition ****************
...
//**************** End AsdSDPCAMAFData Definition ****************

typedef AsdSDPCAMAFData *LPSDPCAMAF;
typedef AsdSDPCAMAFData::AsdTSHeader *LPSDPCAMAF_TSHeader;
typedef AsdSDPCAMAFData::AsdSDAMAFII *LPSDPCAMAF_SDAMAFII;
typedef AsdSDPCAMAFData::AsdTSMDA *LPSDPCAMAF_TSMDA;
typedef AsdSDPCAMAFData::AsdTSMDA::AsdMdaTrmMsg *LPSDPCAMAF_TSMDA_MdaTrmMsg;
typedef AsdSDPCAMAFData::AsdTSMDA::AsdMdaTrmMsg::AsdMdaMsg *LPSDPCAMAF_TSMDA_MdaTrmMsg_MdaMsg;
#endif    // AsdSDPCAMAFData_H
