#ifndef AsdTWPCDTTMData_H
#define AsdTWPCDTTMData_H

#include <stdlib.h>
#ifdef  AsdTWPCDTTM_INTEGRITY_STRING
#undef  AsdTWPCDTTM_INTEGRITY_STRING
#endif
#define AsdTWPCDTTM_INTEGRITY_STRING        ""

#ifdef  AsdTWPCDTTM_CRC_STRING
#undef  AsdTWPCDTTM_CRC_STRING
#endif
#define AsdTWPCDTTM_CRC_STRING "1149009962"
...
//**************** Start AsdTWPCDTTMData Definition ****************
//                 Host Date_Time

//                 TWPCDTTMData.h generated by AceGen2 ver. 2.5.09

class AsdTWPCDTTMData
{
  public:
  ...
  public:
    AsdTWPCDTTMData ()  // Constructors
    {
        InitializeSessionData(&m_Sess);                      // InitializeSessionData declared in bcAce2.h
        m_Sess.pFileName = "TWPCDTTM.ace";                       // This variable is fixed at gen time
        m_Sess.pProtocol = "TLU0";                           // This variable is fixed at gen time
        m_Sess.pIntegrityString = AsdTWPCDTTM_INTEGRITY_STRING;  // This variable is fixed at gen time
    ...
    RWCString HostLUName (void)
    {
        ...
        AddXMLField(&m_Sess, m_Sess.pIntegrityTag, AsdTWPCDTTM_INTEGRITY_STRING, NULL);
    ...
    long Execute (void)
    {
        ...
        AddXMLField(&m_Sess, m_Sess.pCRCTag, AsdTWPCDTTM_CRC_STRING, NULL);
        AddXMLRecord(&m_Sess, m_Sess.pDataTag, -1);       // DATA
        CTSHeader.Save(&m_Sess);
        CSDTIMDTE.Save(&m_Sess);
        CTSMDA.Save(&m_Sess);
        ...
        if ((ret = ExecuteXMLTransaction(&m_Sess)) == 0)
        {
            if (GetXMLRecord(&m_Sess, m_Sess.pAceTag, NULL, NULL))
            {
                ...
                {
                    if(GetXMLRecord(&m_Sess, "TWPCDTTM", NULL, NULL))
                    {
                        CTSHeader.Load(&m_Sess);
                        CSDTIMDTE.Load(&m_Sess);
                        CTSMDA.Load(&m_Sess);
    
                        CloseXMLRecord(&m_Sess, "TWPCDTTM");
                    }
    ...
    //**************** Start AsdSDTIMDTE Definition ****************
    //                 TELLER WORKSTATION HOST TIME/DATE RETRIEVAL                 
    ...
    class AsdSDTIMDTE
    {
      private:
        ...
        char      m_szSuppressBuff[100];
        bool      m_bHostTime;
        char      m_szHostTime[6+1];
        bool      m_bHostDate;
        char      m_szHostDate[8+1];
    
      public:
        long      m_lIndex;

        AsdSDTIMDTE ()  // Constructor
        {
            m_bHostInitOverride = false;
            strcpy(m_szHostInitOverride, "");
            m_bReceived = false;
        ...
        //    HOST TIME
        long lHostTime () { return 6; };
        RWCString HostTime (void)
        {
            return m_szHostTime;
        }
        long HostTime (const RWCString& rwc)
        {
            return SetString(rwc, m_szHostTime, sizeof(m_szHostTime), &m_bHostTime);
        }
        
        //    HOST DATE
        long lHostDate () { return 8; };
        RWCString HostDate (void)
        {
            return m_szHostDate;
        }
        long HostDate (const RWCString& rwc)
        {
            return SetString(rwc, m_szHostDate, sizeof(m_szHostDate), &m_bHostDate);
        }
        
        bool HasDataToSave (void)
        {
            if (m_bHostInitOverride) return true;
            if (m_bHostTime) return true;
            if (m_bHostDate) return true;
            return false;
        }
        ...
        void Save (SESSIONDATA *pSess)
        {
            if (HasDataToSave())
            {
                AddXMLRecord(pSess, "SDTIMDTE", m_lIndex);
                AddXMLField(pSess, "HostInitOverride", m_szHostInitOverride, &m_bHostInitOverride);
                AddXMLField(pSess, "HostTime", m_szHostTime, &m_bHostTime);
                AddXMLField(pSess, "HostDate", m_szHostDate, &m_bHostDate);
                AddXMLCloseRecord(pSess, "SDTIMDTE");  // ACE
            };
        }

        void Load (SESSIONDATA *pSess)
        {
            if (GetXMLRecord(pSess, "SDTIMDTE", m_lIndex, &m_bReceived))
            {
                GetXMLField(pSess, "HostTime", m_szHostTime);
                GetXMLField(pSess, "HostDate", m_szHostDate);
                CloseXMLRecord(pSess, "SDTIMDTE");
            };
        }
    };

    AsdSDTIMDTE CSDTIMDTE;
    AsdSDTIMDTE *SDTIMDTE;
    
    //**************** End AsdSDTIMDTE Definition ****************
...
//**************** End AsdTWPCDTTMData Definition ****************

typedef AsdTWPCDTTMData *LPTWPCDTTM;
typedef AsdTWPCDTTMData::AsdTSHeader *LPTWPCDTTM_TSHeader;
typedef AsdTWPCDTTMData::AsdSDTIMDTE *LPTWPCDTTM_SDTIMDTE;
typedef AsdTWPCDTTMData::AsdTSMDA *LPTWPCDTTM_TSMDA;
typedef AsdTWPCDTTMData::AsdTSMDA::AsdMdaTrmMsg *LPTWPCDTTM_TSMDA_MdaTrmMsg;
typedef AsdTWPCDTTMData::AsdTSMDA::AsdMdaTrmMsg::AsdMdaMsg *LPTWPCDTTM_TSMDA_MdaTrmMsg_MdaMsg;
#endif    // AsdTWPCDTTMData_H
