#ifndef AsdSDPCIMSHData_H
#define AsdSDPCIMSHData_H

#include <stdlib.h>
#ifdef  AsdSDPCIMSH_INTEGRITY_STRING
#undef  AsdSDPCIMSH_INTEGRITY_STRING
#endif
#define AsdSDPCIMSH_INTEGRITY_STRING        ""

#ifdef  AsdSDPCIMSH_CRC_STRING
#undef  AsdSDPCIMSH_CRC_STRING
#endif
#define AsdSDPCIMSH_CRC_STRING "177800882"
...
//**************** Start AsdSDPCIMSHData Definition ****************
//                 IM Stop/Hold Inquiry

//                 SDPCIMSHData.h generated by AceGen2 ver. 2.5.09

class AsdSDPCIMSHData
{
  public:
  ...
  public:
    AsdSDPCIMSHData ()  // Constructors
    {
        InitializeSessionData(&m_Sess);                      // InitializeSessionData declared in bcAce2.h
        m_Sess.pFileName = "SDPCIMSH.ace";                       // This variable is fixed at gen time
        m_Sess.pProtocol = "TLU0";                           // This variable is fixed at gen time
        m_Sess.pIntegrityString = AsdSDPCIMSH_INTEGRITY_STRING;  // This variable is fixed at gen time
    ...
    RWCString HostLUName (void)
    {
        ...
        AddXMLField(&m_Sess, m_Sess.pIntegrityTag, AsdSDPCIMSH_INTEGRITY_STRING, NULL);
    ...
    long Execute (void)
    {
        ...
        AddXMLField(&m_Sess, m_Sess.pCRCTag, AsdSDPCIMSH_CRC_STRING, NULL);
        AddXMLRecord(&m_Sess, m_Sess.pDataTag, -1);       // DATA
        CTSHeader.Save(&m_Sess);
        CSDIMCMST.Save(&m_Sess);
        CSDIMSHIQ.Save(&m_Sess);
        CTSMDA.Save(&m_Sess);
        ...
        if ((ret = ExecuteXMLTransaction(&m_Sess)) == 0)
        {
            if (GetXMLRecord(&m_Sess, m_Sess.pAceTag, NULL, NULL))
            {
                ...
                {
                    if(GetXMLRecord(&m_Sess, "SDPCIMSH", NULL, NULL))
                    {
                        CTSHeader.Load(&m_Sess);
                        CSDIMCMST.Load(&m_Sess);
                        CSDIMSHIQ.Load(&m_Sess);
                        CTSMDA.Load(&m_Sess);
    
                        CloseXMLRecord(&m_Sess, "SDPCIMSH");
                    }
    ...
    class AsdSDIMCMST
    {
      ...
      public:
        ...
        void Save (SESSIONDATA *pSess)
        {
            if (HasDataToSave())
            {
                AddXMLRecord(pSess, "SDIMCMST", m_lIndex);
                AddXMLField(pSess, "HostInitOverride", m_szHostInitOverride, &m_bHostInitOverride);
                ...
                AddXMLCloseRecord(pSess, "SDIMCMST");  // ACE
            };
        }

        void Load (SESSIONDATA *pSess)
        {
            if (GetXMLRecord(pSess, "SDIMCMST", m_lIndex, &m_bReceived))
            {
                ...
                CloseXMLRecord(pSess, "SDIMCMST");
            };
        }
    };

    AsdSDIMCMST CSDIMCMST;
    AsdSDIMCMST *SDIMCMST;
    
    //**************** End AsdSDIMCMST Definition ****************

    
    //**************** Start AsdSDIMSHIQ Definition ****************
    //                 SD STOP/HOLD INQUIRY FOR IM                                 
    ...
    class AsdSDIMSHIQ
    {
      ...
      public:
        ...
        AsdSDIMSHIQ ()  // Constructor
        {
            ...
            strcpy(m_szSaveSeq, "+0000000");
            m_bShNbr = false;
            strcpy(m_szShNbr, "+00000");
            long lOccurs;
        
            for (lOccurs = 0; lOccurs < 50; lOccurs++)
            {
                ShOccurance[lOccurs]  = &CShOccurance[lOccurs];
                CShOccurance[lOccurs].m_lIndex = lOccurs;
            }
        }
        ...
        //    NUMBER OF STOPS/HOLDS/SPECIAL INSTRUCTIONS
        long lShNbr () { return 6; };
        RWCString ShNbr (void)
        {
            return AceZeroSupp(m_szShNbr, &m_szSuppressBuff[0]);
        }
        RWCString ShNbrRaw (void)
        {
            return m_szShNbr;
        }
        long ShNbr (const RWCString& rwc)
        {
            return SetString(rwc, m_szShNbr, sizeof(m_szShNbr), &m_bShNbr);
        }
        ...
        bool HasDataToSave (void)
        {
            ...
            if (m_bProcessFlag) return true;
            if (m_bSaveSeq) return true;
            if (m_bShNbr) return true;
            long lOccurs;
            for (lOccurs = 0; lOccurs < 50; lOccurs++)
            {
                if(CShOccurance[lOccurs].HasDataToSave())
                {
                    return true;
                }
            }
        ...
        void Save (SESSIONDATA *pSess)
        {
            if (HasDataToSave())
            {
                AddXMLRecord(pSess, "SDIMSHIQ", m_lIndex);
                AddXMLField(pSess, "HostInitOverride", m_szHostInitOverride, &m_bHostInitOverride);
                AddXMLField(pSess, "ProcessFlag", m_szProcessFlag, &m_bProcessFlag);
                AddXMLField(pSess, "SaveSeq", m_szSaveSeq, &m_bSaveSeq);
                AddXMLField(pSess, "ShNbr", m_szShNbr, &m_bShNbr);
                long lOccurs;
                for (lOccurs = 0; lOccurs < 50; lOccurs++)
                {
                    CShOccurance[lOccurs].Save(pSess);
                }
                AddXMLCloseRecord(pSess, "SDIMSHIQ");  // ACE
            };
        }

        void Load (SESSIONDATA *pSess)
        {
            if (GetXMLRecord(pSess, "SDIMSHIQ", m_lIndex, &m_bReceived))
            {
                GetXMLField(pSess, "ProcessFlag", m_szProcessFlag);
                GetXMLField(pSess, "SaveSeq", m_szSaveSeq);
                GetXMLField(pSess, "ShNbr", m_szShNbr);
                long lOccurs;
                for (lOccurs = 0; lOccurs < 50; lOccurs++)
                {
                    CShOccurance[lOccurs].Load(pSess);
                }
                CloseXMLRecord(pSess, "SDIMSHIQ");
            };
        }
        
        //**************** Start AsdShOccurance Definition ****************
        //                 STOP/HOLD OCCURANCE
        
        class AsdShOccurance
        {
          private:
            bool      m_bReceived;
            char      m_szSuppressBuff[100];
            ...
            char      m_szShType[1+1];
            bool      m_bShAmount;
            char      m_szShAmount[15+1];
            bool      m_bShLowCkNo;
            char      m_szShLowCkNo[10+1];
            bool      m_bShExpCent;
            char      m_szShExpCent[2+1];
            bool      m_bShExpYr;
            char      m_szShExpYr[2+1];
            bool      m_bShExpMo;
            char      m_szShExpMo[2+1];
            bool      m_bShExpDa;
            char      m_szShExpDa[2+1];
            bool      m_bShHighCkNo;
            char      m_szShHighCkNo[10+1];
            bool      m_bShSubType;
            char      m_szShSubType[1+1];
            bool      m_bShInitCent;
            char      m_szShInitCent[2+1];
            bool      m_bShInitYr;
            char      m_szShInitYr[2+1];
            bool      m_bShInitMo;
            char      m_szShInitMo[2+1];
            bool      m_bShInitDa;
            char      m_szShInitDa[2+1];
            bool      m_bShOrigDays;
            char      m_szShOrigDays[4+1];
            bool      m_bShRemDays;
            char      m_szShRemDays[4+1];
            bool      m_bShDescription;
            char      m_szShDescription[24+1];
            bool      m_bShGenInd;
            char      m_szShGenInd[1+1];
            bool      m_bShSpecInstFlag;
            char      m_szShSpecInstFlag[1+1];
            bool      m_bShSpecInstDesc;
            char      m_szShSpecInstDesc[46+1];
            bool      m_bShReason;
            char      m_szShReason[1+1];
            bool      m_bShOnlineDelInd;
            char      m_szShOnlineDelInd[1+1];
            bool      m_bShSrceInfoFlag;
            char      m_szShSrceInfoFlag[1+1];
            bool      m_bShSourceOperId;
            char      m_szShSourceOperId[8+1];
            bool      m_bShSourceBranch;
            char      m_szShSourceBranch[3+1];
            bool      m_bShSourceTermId;
            char      m_szShSourceTermId[4+1];
            bool      m_bShSourceDate;
            char      m_szShSourceDate[8+1];
            bool      m_bShSourceTime;
            char      m_szShSourceTime[8+1];
            bool      m_bShSourceOrigin;
            char      m_szShSourceOrigin[2+1];
        
          public:
            long      m_lIndex;

            AsdShOccurance ()  // Constructor
            {
                m_bReceived = false;
                m_lIndex = -1;
                m_bShSeq = false;
                strcpy(m_szShSeq, "+00000");
                m_bShType = false;
                strcpy(m_szShType, "");
                m_bShAmount = false;
                strcpy(m_szShAmount, "+00000000000.00");
                m_bShLowCkNo = false;
                strcpy(m_szShLowCkNo, "");
                m_bShExpCent = false;
                strcpy(m_szShExpCent, "");
                m_bShExpYr = false;
                strcpy(m_szShExpYr, "");
                m_bShExpMo = false;
                strcpy(m_szShExpMo, "");
                ...
                strcpy(m_szShExpDa, "");
                m_bShHighCkNo = false;
                strcpy(m_szShHighCkNo, "");
                m_bShSubType = false;
                strcpy(m_szShSubType, "");
                m_bShInitCent = false;
                strcpy(m_szShInitCent, "");
                m_bShInitYr = false;
                strcpy(m_szShInitYr, "");
                m_bShInitMo = false;
                strcpy(m_szShInitMo, "");
                ...
                strcpy(m_szShInitDa, "");
                m_bShOrigDays = false;
                strcpy(m_szShOrigDays, "+000");
                m_bShRemDays = false;
                strcpy(m_szShRemDays, "+000");
                m_bShDescription = false;
                strcpy(m_szShDescription, "");
                m_bShGenInd = false;
                strcpy(m_szShGenInd, "");
                m_bShSpecInstFlag = false;
                strcpy(m_szShSpecInstFlag, "");
                m_bShSpecInstDesc = false;
                strcpy(m_szShSpecInstDesc, "");
                m_bShReason = false;
                strcpy(m_szShReason, "");
                m_bShOnlineDelInd = false;
                strcpy(m_szShOnlineDelInd, "");
                m_bShSrceInfoFlag = false;
                strcpy(m_szShSrceInfoFlag, "");
                m_bShSourceOperId = false;
                strcpy(m_szShSourceOperId, "");
                m_bShSourceBranch = false;
                strcpy(m_szShSourceBranch, "");
                m_bShSourceTermId = false;
                strcpy(m_szShSourceTermId, "");
                m_bShSourceDate = false;
                strcpy(m_szShSourceDate, "");
                m_bShSourceTime = false;
                strcpy(m_szShSourceTime, "+0000000");
                m_bShSourceOrigin = false;
                strcpy(m_szShSourceOrigin, "");
            ...
            long lShSeq () { return 6; };
            RWCString ShSeq (void)
            {
                return AceZeroSupp(m_szShSeq, &m_szSuppressBuff[0]);
            }
            RWCString ShSeqRaw (void)
            {
                return m_szShSeq;
            }
            long ShSeq (const RWCString& rwc)
            {
                return SetString(rwc, m_szShSeq, sizeof(m_szShSeq), &m_bShSeq);
            }
            
            //    TYPE OF STOP/HOLD
            long lShType () { return 1; };
            RWCString ShType (void)
            {
                return m_szShType;
            }
            long ShType (const RWCString& rwc)
            {
                return SetString(rwc, m_szShType, sizeof(m_szShType), &m_bShType);
            }
            
            //    STOP/HOLD AMOUNT
            long lShAmount () { return 15; };
            RWCString ShAmount (void)
            {
                return AceZeroSupp(m_szShAmount, &m_szSuppressBuff[0]);
            }
            RWCString ShAmountRaw (void)
            {
                return m_szShAmount;
            }
            long ShAmount (const RWCString& rwc)
            {
                return SetString(rwc, m_szShAmount, sizeof(m_szShAmount), &m_bShAmount);
            }
            
            //    LOW CHECK NUMBER FOR A STOP RANGE
            long lShLowCkNo () { return 10; };
            RWCString ShLowCkNo (void)
            {
                return m_szShLowCkNo;
            }
            long ShLowCkNo (const RWCString& rwc)
            {
                return SetString(rwc, m_szShLowCkNo, sizeof(m_szShLowCkNo), &m_bShLowCkNo);
            }
            
            //    EXPIRATION DATE CENTURY
            long lShExpCent () { return 2; };
            RWCString ShExpCent (void)
            {
                return m_szShExpCent;
            }
            long ShExpCent (const RWCString& rwc)
            {
                return SetString(rwc, m_szShExpCent, sizeof(m_szShExpCent), &m_bShExpCent);
            }
            
            //    EXPIRATION DATE YEAR
            long lShExpYr () { return 2; };
            RWCString ShExpYr (void)
            {
                return m_szShExpYr;
            }
            long ShExpYr (const RWCString& rwc)
            {
                return SetString(rwc, m_szShExpYr, sizeof(m_szShExpYr), &m_bShExpYr);
            }
            
            //    EXPIRATION DATE MONTH
            long lShExpMo () { return 2; };
            RWCString ShExpMo (void)
            {
                return m_szShExpMo;
            }
            long ShExpMo (const RWCString& rwc)
            {
                return SetString(rwc, m_szShExpMo, sizeof(m_szShExpMo), &m_bShExpMo);
            }
            
            //    EXPIRATION DATE DAY
            long lShExpDa () { return 2; };
            RWCString ShExpDa (void)
            {
                return m_szShExpDa;
            }
            long ShExpDa (const RWCString& rwc)
            {
                return SetString(rwc, m_szShExpDa, sizeof(m_szShExpDa), &m_bShExpDa);
            }
            
            //    HIGH CHECK NUMBER FOR STOP RANGE
            long lShHighCkNo () { return 10; };
            RWCString ShHighCkNo (void)
            {
                return m_szShHighCkNo;
            }
            long ShHighCkNo (const RWCString& rwc)
            {
                return SetString(rwc, m_szShHighCkNo, sizeof(m_szShHighCkNo), &m_bShHighCkNo);
            }
            
            //    SUB TYPE OF STOP/HOLD
            long lShSubType () { return 1; };
            RWCString ShSubType (void)
            {
                return m_szShSubType;
            }
            long ShSubType (const RWCString& rwc)
            {
                return SetString(rwc, m_szShSubType, sizeof(m_szShSubType), &m_bShSubType);
            }
            
            //    INITIAL DATE CENTURY
            long lShInitCent () { return 2; };
            RWCString ShInitCent (void)
            {
                return m_szShInitCent;
            }
            long ShInitCent (const RWCString& rwc)
            {
                return SetString(rwc, m_szShInitCent, sizeof(m_szShInitCent), &m_bShInitCent);
            }
            
            //    INITIAL DATE YEAR
            long lShInitYr () { return 2; };
            RWCString ShInitYr (void)
            {
                return m_szShInitYr;
            }
            long ShInitYr (const RWCString& rwc)
            {
                return SetString(rwc, m_szShInitYr, sizeof(m_szShInitYr), &m_bShInitYr);
            }
            
            //    INITIAL DATE MONTH
            long lShInitMo () { return 2; };
            RWCString ShInitMo (void)
            {
                return m_szShInitMo;
            }
            long ShInitMo (const RWCString& rwc)
            {
                return SetString(rwc, m_szShInitMo, sizeof(m_szShInitMo), &m_bShInitMo);
            }
            
            //    INITIAL DATE DAY
            long lShInitDa () { return 2; };
            RWCString ShInitDa (void)
            {
                return m_szShInitDa;
            }
            long ShInitDa (const RWCString& rwc)
            {
                return SetString(rwc, m_szShInitDa, sizeof(m_szShInitDa), &m_bShInitDa);
            }
            
            //    ORIGINAL NUMBER OF DAYS FOR THE ACTION TO BE IN EFFECT
            long lShOrigDays () { return 4; };
            RWCString ShOrigDays (void)
            {
                return AceZeroSupp(m_szShOrigDays, &m_szSuppressBuff[0]);
            }
            RWCString ShOrigDaysRaw (void)
            {
                return m_szShOrigDays;
            }
            long ShOrigDays (const RWCString& rwc)
            {
                return SetString(rwc, m_szShOrigDays, sizeof(m_szShOrigDays), &m_bShOrigDays);
            }
            
            //    NUMBER OF DAYS REMAINING FOR THE ACTION TO BE IN EFFECT
            long lShRemDays () { return 4; };
            RWCString ShRemDays (void)
            {
                return AceZeroSupp(m_szShRemDays, &m_szSuppressBuff[0]);
            }
            RWCString ShRemDaysRaw (void)
            {
                return m_szShRemDays;
            }
            long ShRemDays (const RWCString& rwc)
            {
                return SetString(rwc, m_szShRemDays, sizeof(m_szShRemDays), &m_bShRemDays);
            }
            
            //    DESCRIPTION
            long lShDescription () { return 24; };
            RWCString ShDescription (void)
            {
                return m_szShDescription;
            }
            long ShDescription (const RWCString& rwc)
            {
                return SetString(rwc, m_szShDescription, sizeof(m_szShDescription), &m_bShDescription);
            }
            
            //    CUSTOMER GENERATED
            long lShGenInd () { return 1; };
            RWCString ShGenInd (void)
            {
                return m_szShGenInd;
            }
            long ShGenInd (const RWCString& rwc)
            {
                return SetString(rwc, m_szShGenInd, sizeof(m_szShGenInd), &m_bShGenInd);
            }
            
            //    SPECIAL INSTRUCTION FLAG
            long lShSpecInstFlag () { return 1; };
            RWCString ShSpecInstFlag (void)
            {
                return m_szShSpecInstFlag;
            }
            long ShSpecInstFlag (const RWCString& rwc)
            {
                return SetString(rwc, m_szShSpecInstFlag, sizeof(m_szShSpecInstFlag), &m_bShSpecInstFlag);
            }
            
            //    SPECIAL INSTRUCTION DESCRIPTION
            long lShSpecInstDesc () { return 46; };
            RWCString ShSpecInstDesc (void)
            {
                return m_szShSpecInstDesc;
            }
            long ShSpecInstDesc (const RWCString& rwc)
            {
                return SetString(rwc, m_szShSpecInstDesc, sizeof(m_szShSpecInstDesc), &m_bShSpecInstDesc);
            }
            
            //    REASON
            long lShReason () { return 1; };
            RWCString ShReason (void)
            {
                return m_szShReason;
            }
            long ShReason (const RWCString& rwc)
            {
                return SetString(rwc, m_szShReason, sizeof(m_szShReason), &m_bShReason);
            }
            
            //    ONLINE DELETE INDICATOR
            long lShOnlineDelInd () { return 1; };
            RWCString ShOnlineDelInd (void)
            {
                return m_szShOnlineDelInd;
            }
            long ShOnlineDelInd (const RWCString& rwc)
            {
                return SetString(rwc, m_szShOnlineDelInd, sizeof(m_szShOnlineDelInd), &m_bShOnlineDelInd);
            }
            
            //    FLAG INDICATING ONLINE INFORMATION AVAILABLE
            long lShSrceInfoFlag () { return 1; };
            RWCString ShSrceInfoFlag (void)
            {
                return m_szShSrceInfoFlag;
            }
            long ShSrceInfoFlag (const RWCString& rwc)
            {
                return SetString(rwc, m_szShSrceInfoFlag, sizeof(m_szShSrceInfoFlag), &m_bShSrceInfoFlag);
            }
            
            //    SOURCE OPERATOR IDENTIFICATION
            long lShSourceOperId () { return 8; };
            RWCString ShSourceOperId (void)
            {
                return m_szShSourceOperId;
            }
            long ShSourceOperId (const RWCString& rwc)
            {
                return SetString(rwc, m_szShSourceOperId, sizeof(m_szShSourceOperId), &m_bShSourceOperId);
            }
            
            //    SOURCE BRANCH
            long lShSourceBranch () { return 3; };
            RWCString ShSourceBranch (void)
            {
                return m_szShSourceBranch;
            }
            long ShSourceBranch (const RWCString& rwc)
            {
                return SetString(rwc, m_szShSourceBranch, sizeof(m_szShSourceBranch), &m_bShSourceBranch);
            }
            
            //    SOURCE TERMINAL ID
            long lShSourceTermId () { return 4; };
            RWCString ShSourceTermId (void)
            {
                return m_szShSourceTermId;
            }
            long ShSourceTermId (const RWCString& rwc)
            {
                return SetString(rwc, m_szShSourceTermId, sizeof(m_szShSourceTermId), &m_bShSourceTermId);
            }
            
            //    SOURCE DATE
            long lShSourceDate () { return 8; };
            RWCString ShSourceDate (void)
            {
                return m_szShSourceDate;
            }
            long ShSourceDate (const RWCString& rwc)
            {
                return SetString(rwc, m_szShSourceDate, sizeof(m_szShSourceDate), &m_bShSourceDate);
            }
            
            //    SOURCE TIME
            long lShSourceTime () { return 8; };
            RWCString ShSourceTime (void)
            {
                return AceZeroSupp(m_szShSourceTime, &m_szSuppressBuff[0]);
            }
            RWCString ShSourceTimeRaw (void)
            {
                return m_szShSourceTime;
            }
            long ShSourceTime (const RWCString& rwc)
            {
                return SetString(rwc, m_szShSourceTime, sizeof(m_szShSourceTime), &m_bShSourceTime);
            }
            
            //    SOURCE ORIGIN
            long lShSourceOrigin () { return 2; };
            RWCString ShSourceOrigin (void)
            {
                return m_szShSourceOrigin;
            }
            long ShSourceOrigin (const RWCString& rwc)
            {
                return SetString(rwc, m_szShSourceOrigin, sizeof(m_szShSourceOrigin), &m_bShSourceOrigin);
            }
            
            bool HasDataToSave (void)
            {
                if (m_bShSeq) return true;
                if (m_bShType) return true;
                if (m_bShAmount) return true;
                if (m_bShLowCkNo) return true;
                if (m_bShExpCent) return true;
                if (m_bShExpYr) return true;
                if (m_bShExpMo) return true;
                if (m_bShExpDa) return true;
                if (m_bShHighCkNo) return true;
                if (m_bShSubType) return true;
                if (m_bShInitCent) return true;
                if (m_bShInitYr) return true;
                if (m_bShInitMo) return true;
                if (m_bShInitDa) return true;
                if (m_bShOrigDays) return true;
                if (m_bShRemDays) return true;
                if (m_bShDescription) return true;
                if (m_bShGenInd) return true;
                if (m_bShSpecInstFlag) return true;
                if (m_bShSpecInstDesc) return true;
                if (m_bShReason) return true;
                if (m_bShOnlineDelInd) return true;
                if (m_bShSrceInfoFlag) return true;
                if (m_bShSourceOperId) return true;
                if (m_bShSourceBranch) return true;
                if (m_bShSourceTermId) return true;
                if (m_bShSourceDate) return true;
                if (m_bShSourceTime) return true;
                if (m_bShSourceOrigin) return true;
                return false;
            }
            ...
            void Save (SESSIONDATA *pSess)
            {
                if (HasDataToSave())
                {
                    AddXMLRecord(pSess, "ShOccurance", m_lIndex);
                    AddXMLField(pSess, "ShSeq", m_szShSeq, &m_bShSeq);
                    AddXMLField(pSess, "ShType", m_szShType, &m_bShType);
                    AddXMLField(pSess, "ShAmount", m_szShAmount, &m_bShAmount);
                    AddXMLField(pSess, "ShLowCkNo", m_szShLowCkNo, &m_bShLowCkNo);
                    AddXMLField(pSess, "ShExpCent", m_szShExpCent, &m_bShExpCent);
                    AddXMLField(pSess, "ShExpYr", m_szShExpYr, &m_bShExpYr);
                    AddXMLField(pSess, "ShExpMo", m_szShExpMo, &m_bShExpMo);
                    AddXMLField(pSess, "ShExpDa", m_szShExpDa, &m_bShExpDa);
                    AddXMLField(pSess, "ShHighCkNo", m_szShHighCkNo, &m_bShHighCkNo);
                    AddXMLField(pSess, "ShSubType", m_szShSubType, &m_bShSubType);
                    AddXMLField(pSess, "ShInitCent", m_szShInitCent, &m_bShInitCent);
                    AddXMLField(pSess, "ShInitYr", m_szShInitYr, &m_bShInitYr);
                    AddXMLField(pSess, "ShInitMo", m_szShInitMo, &m_bShInitMo);
                    AddXMLField(pSess, "ShInitDa", m_szShInitDa, &m_bShInitDa);
                    AddXMLField(pSess, "ShOrigDays", m_szShOrigDays, &m_bShOrigDays);
                    AddXMLField(pSess, "ShRemDays", m_szShRemDays, &m_bShRemDays);
                    AddXMLField(pSess, "ShDescription", m_szShDescription, &m_bShDescription);
                    AddXMLField(pSess, "ShGenInd", m_szShGenInd, &m_bShGenInd);
                    AddXMLField(pSess, "ShSpecInstFlag", m_szShSpecInstFlag, &m_bShSpecInstFlag);
                    AddXMLField(pSess, "ShSpecInstDesc", m_szShSpecInstDesc, &m_bShSpecInstDesc);
                    AddXMLField(pSess, "ShReason", m_szShReason, &m_bShReason);
                    AddXMLField(pSess, "ShOnlineDelInd", m_szShOnlineDelInd, &m_bShOnlineDelInd);
                    AddXMLField(pSess, "ShSrceInfoFlag", m_szShSrceInfoFlag, &m_bShSrceInfoFlag);
                    AddXMLField(pSess, "ShSourceOperId", m_szShSourceOperId, &m_bShSourceOperId);
                    AddXMLField(pSess, "ShSourceBranch", m_szShSourceBranch, &m_bShSourceBranch);
                    AddXMLField(pSess, "ShSourceTermId", m_szShSourceTermId, &m_bShSourceTermId);
                    AddXMLField(pSess, "ShSourceDate", m_szShSourceDate, &m_bShSourceDate);
                    AddXMLField(pSess, "ShSourceTime", m_szShSourceTime, &m_bShSourceTime);
                    AddXMLField(pSess, "ShSourceOrigin", m_szShSourceOrigin, &m_bShSourceOrigin);
                    AddXMLCloseRecord(pSess, "ShOccurance");  // ACE
                };
            }

            void Load (SESSIONDATA *pSess)
            {
                if (GetXMLRecord(pSess, "ShOccurance", m_lIndex, &m_bReceived))
                {
                    GetXMLField(pSess, "ShSeq", m_szShSeq);
                    GetXMLField(pSess, "ShType", m_szShType);
                    GetXMLField(pSess, "ShAmount", m_szShAmount);
                    GetXMLField(pSess, "ShLowCkNo", m_szShLowCkNo);
                    GetXMLField(pSess, "ShExpCent", m_szShExpCent);
                    GetXMLField(pSess, "ShExpYr", m_szShExpYr);
                    GetXMLField(pSess, "ShExpMo", m_szShExpMo);
                    GetXMLField(pSess, "ShExpDa", m_szShExpDa);
                    GetXMLField(pSess, "ShHighCkNo", m_szShHighCkNo);
                    GetXMLField(pSess, "ShSubType", m_szShSubType);
                    GetXMLField(pSess, "ShInitCent", m_szShInitCent);
                    GetXMLField(pSess, "ShInitYr", m_szShInitYr);
                    GetXMLField(pSess, "ShInitMo", m_szShInitMo);
                    GetXMLField(pSess, "ShInitDa", m_szShInitDa);
                    GetXMLField(pSess, "ShOrigDays", m_szShOrigDays);
                    GetXMLField(pSess, "ShRemDays", m_szShRemDays);
                    GetXMLField(pSess, "ShDescription", m_szShDescription);
                    GetXMLField(pSess, "ShGenInd", m_szShGenInd);
                    GetXMLField(pSess, "ShSpecInstFlag", m_szShSpecInstFlag);
                    GetXMLField(pSess, "ShSpecInstDesc", m_szShSpecInstDesc);
                    GetXMLField(pSess, "ShReason", m_szShReason);
                    GetXMLField(pSess, "ShOnlineDelInd", m_szShOnlineDelInd);
                    GetXMLField(pSess, "ShSrceInfoFlag", m_szShSrceInfoFlag);
                    GetXMLField(pSess, "ShSourceOperId", m_szShSourceOperId);
                    GetXMLField(pSess, "ShSourceBranch", m_szShSourceBranch);
                    GetXMLField(pSess, "ShSourceTermId", m_szShSourceTermId);
                    GetXMLField(pSess, "ShSourceDate", m_szShSourceDate);
                    GetXMLField(pSess, "ShSourceTime", m_szShSourceTime);
                    GetXMLField(pSess, "ShSourceOrigin", m_szShSourceOrigin);
                    CloseXMLRecord(pSess, "ShOccurance");
                };
            }
        };

        AsdShOccurance  CShOccurance[50];
        AsdShOccurance *ShOccurance[50];
        
        //**************** End AsdShOccurance Definition ****************
        
    };

    AsdSDIMSHIQ CSDIMSHIQ;
    AsdSDIMSHIQ *SDIMSHIQ;
    
    //**************** End AsdSDIMSHIQ Definition ****************
...
//**************** End AsdSDPCIMSHData Definition ****************

typedef AsdSDPCIMSHData *LPSDPCIMSH;
typedef AsdSDPCIMSHData::AsdTSHeader *LPSDPCIMSH_TSHeader;
typedef AsdSDPCIMSHData::AsdSDIMCMST *LPSDPCIMSH_SDIMCMST;
typedef AsdSDPCIMSHData::AsdSDIMSHIQ *LPSDPCIMSH_SDIMSHIQ;
typedef AsdSDPCIMSHData::AsdSDIMSHIQ::AsdShOccurance *LPSDPCIMSH_SDIMSHIQ_ShOccurance;
typedef AsdSDPCIMSHData::AsdTSMDA *LPSDPCIMSH_TSMDA;
typedef AsdSDPCIMSHData::AsdTSMDA::AsdMdaTrmMsg *LPSDPCIMSH_TSMDA_MdaTrmMsg;
typedef AsdSDPCIMSHData::AsdTSMDA::AsdMdaTrmMsg::AsdMdaMsg *LPSDPCIMSH_TSMDA_MdaTrmMsg_MdaMsg;
#endif    // AsdSDPCIMSHData_H
