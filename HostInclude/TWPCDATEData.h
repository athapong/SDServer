#ifndef AsdTWPCDATEData_H
#define AsdTWPCDATEData_H

#include <stdlib.h>
#ifdef  AsdTWPCDATE_INTEGRITY_STRING
#undef  AsdTWPCDATE_INTEGRITY_STRING
#endif
#define AsdTWPCDATE_INTEGRITY_STRING        ""

#ifdef  AsdTWPCDATE_CRC_STRING
#undef  AsdTWPCDATE_CRC_STRING
#endif
#define AsdTWPCDATE_CRC_STRING "3873623057"
...
//**************** Start AsdTWPCDATEData Definition ****************
//                 Host Date Download

//                 TWPCDATEData.h generated by AceGen2 ver. 2.5.09

class AsdTWPCDATEData
{
  public:
  ...
  public:
    AsdTWPCDATEData ()  // Constructors
    {
        InitializeSessionData(&m_Sess);                      // InitializeSessionData declared in bcAce2.h
        m_Sess.pFileName = "TWPCDATE.ace";                       // This variable is fixed at gen time
        m_Sess.pProtocol = "TLU0";                           // This variable is fixed at gen time
        m_Sess.pIntegrityString = AsdTWPCDATE_INTEGRITY_STRING;  // This variable is fixed at gen time
    ...
    RWCString HostLUName (void)
    {
        ...
        AddXMLField(&m_Sess, m_Sess.pIntegrityTag, AsdTWPCDATE_INTEGRITY_STRING, NULL);
    ...
    long Execute (void)
    {
        ...
        AddXMLField(&m_Sess, m_Sess.pCRCTag, AsdTWPCDATE_CRC_STRING, NULL);
        AddXMLRecord(&m_Sess, m_Sess.pDataTag, -1);       // DATA
        CTSHeader.Save(&m_Sess);
        CSDDATES.Save(&m_Sess);
        CTSMDA.Save(&m_Sess);
        ...
        if ((ret = ExecuteXMLTransaction(&m_Sess)) == 0)
        {
            if (GetXMLRecord(&m_Sess, m_Sess.pAceTag, NULL, NULL))
            {
                ...
                {
                    if(GetXMLRecord(&m_Sess, "TWPCDATE", NULL, NULL))
                    {
                        CTSHeader.Load(&m_Sess);
                        CSDDATES.Load(&m_Sess);
                        CTSMDA.Load(&m_Sess);
    
                        CloseXMLRecord(&m_Sess, "TWPCDATE");
                    }
    ...
    //**************** Start AsdSDDATES Definition ****************
    //                 TELLER WORKSTATION HOLIDAY/PROC DATES RETRIEVAL             
    ...
    class AsdSDDATES
    {
      private:
        ...
        bool      m_bReceived;
        char      m_szSuppressBuff[100];
        bool      m_bPrevProcessDate;
        char      m_szPrevProcessDate[8+1];
        bool      m_bCurrProcessDate;
        char      m_szCurrProcessDate[8+1];
        bool      m_bNextBusinessDate;
        char      m_szNextBusinessDate[8+1];
        bool      m_bProcessSaturday;
        char      m_szProcessSaturday[1+1];
        bool      m_bProcessSunday;
        char      m_szProcessSunday[1+1];
        bool      m_bCalendarNumber;
        char      m_szCalendarNumber[2+1];
    
      public:
        long      m_lIndex;

        AsdSDDATES ()  // Constructor
        {
            m_bHostInitOverride = false;
            strcpy(m_szHostInitOverride, "");
            m_bReceived = false;
            m_lIndex = -1;
            m_bPrevProcessDate = false;
            strcpy(m_szPrevProcessDate, "");
            m_bCurrProcessDate = false;
            strcpy(m_szCurrProcessDate, "");
            m_bNextBusinessDate = false;
            strcpy(m_szNextBusinessDate, "");
            m_bProcessSaturday = false;
            strcpy(m_szProcessSaturday, "");
            m_bProcessSunday = false;
            strcpy(m_szProcessSunday, "");
            m_bCalendarNumber = false;
            strcpy(m_szCalendarNumber, "");
            long lOccurs;
        
            for (lOccurs = 0; lOccurs < 20; lOccurs++)
            {
                HolidayTable[lOccurs]  = &CHolidayTable[lOccurs];
                CHolidayTable[lOccurs].m_lIndex = lOccurs;
            }
        }
        ...
        //    SER PREVIOUS PROCESS DATE
        long lPrevProcessDate () { return 8; };
        RWCString PrevProcessDate (void)
        {
            return m_szPrevProcessDate;
        }
        long PrevProcessDate (const RWCString& rwc)
        {
            return SetString(rwc, m_szPrevProcessDate, sizeof(m_szPrevProcessDate), &m_bPrevProcessDate);
        }
        
        //    SER CURRENT PROCESS DATE
        long lCurrProcessDate () { return 8; };
        RWCString CurrProcessDate (void)
        {
            return m_szCurrProcessDate;
        }
        long CurrProcessDate (const RWCString& rwc)
        {
            return SetString(rwc, m_szCurrProcessDate, sizeof(m_szCurrProcessDate), &m_bCurrProcessDate);
        }
        
        //    SER NEXT BUSINESS DATE
        long lNextBusinessDate () { return 8; };
        RWCString NextBusinessDate (void)
        {
            return m_szNextBusinessDate;
        }
        long NextBusinessDate (const RWCString& rwc)
        {
            return SetString(rwc, m_szNextBusinessDate, sizeof(m_szNextBusinessDate), &m_bNextBusinessDate);
        }
        
        //    PROCESS SATURDAY FLAG
        long lProcessSaturday () { return 1; };
        RWCString ProcessSaturday (void)
        {
            return m_szProcessSaturday;
        }
        long ProcessSaturday (const RWCString& rwc)
        {
            return SetString(rwc, m_szProcessSaturday, sizeof(m_szProcessSaturday), &m_bProcessSaturday);
        }
        
        //    PROCESS SUNDAY FLAG
        long lProcessSunday () { return 1; };
        RWCString ProcessSunday (void)
        {
            return m_szProcessSunday;
        }
        long ProcessSunday (const RWCString& rwc)
        {
            return SetString(rwc, m_szProcessSunday, sizeof(m_szProcessSunday), &m_bProcessSunday);
        }
        
        //    CALENDAR NUMBER
        long lCalendarNumber () { return 2; };
        RWCString CalendarNumber (void)
        {
            return m_szCalendarNumber;
        }
        long CalendarNumber (const RWCString& rwc)
        {
            return SetString(rwc, m_szCalendarNumber, sizeof(m_szCalendarNumber), &m_bCalendarNumber);
        }
        
        bool HasDataToSave (void)
        {
            if (m_bHostInitOverride) return true;
            if (m_bPrevProcessDate) return true;
            if (m_bCurrProcessDate) return true;
            if (m_bNextBusinessDate) return true;
            if (m_bProcessSaturday) return true;
            if (m_bProcessSunday) return true;
            if (m_bCalendarNumber) return true;
            long lOccurs;
            for (lOccurs = 0; lOccurs < 20; lOccurs++)
            {
                if(CHolidayTable[lOccurs].HasDataToSave())
                {
                    return true;
                }
            }
        ...
        void Save (SESSIONDATA *pSess)
        {
            if (HasDataToSave())
            {
                AddXMLRecord(pSess, "SDDATES", m_lIndex);
                AddXMLField(pSess, "HostInitOverride", m_szHostInitOverride, &m_bHostInitOverride);
                AddXMLField(pSess, "PrevProcessDate", m_szPrevProcessDate, &m_bPrevProcessDate);
                AddXMLField(pSess, "CurrProcessDate", m_szCurrProcessDate, &m_bCurrProcessDate);
                AddXMLField(pSess, "NextBusinessDate", m_szNextBusinessDate, &m_bNextBusinessDate);
                AddXMLField(pSess, "ProcessSaturday", m_szProcessSaturday, &m_bProcessSaturday);
                AddXMLField(pSess, "ProcessSunday", m_szProcessSunday, &m_bProcessSunday);
                AddXMLField(pSess, "CalendarNumber", m_szCalendarNumber, &m_bCalendarNumber);
                long lOccurs;
                for (lOccurs = 0; lOccurs < 20; lOccurs++)
                {
                    CHolidayTable[lOccurs].Save(pSess);
                }
                AddXMLCloseRecord(pSess, "SDDATES");  // ACE
            };
        }

        void Load (SESSIONDATA *pSess)
        {
            if (GetXMLRecord(pSess, "SDDATES", m_lIndex, &m_bReceived))
            {
                GetXMLField(pSess, "PrevProcessDate", m_szPrevProcessDate);
                GetXMLField(pSess, "CurrProcessDate", m_szCurrProcessDate);
                GetXMLField(pSess, "NextBusinessDate", m_szNextBusinessDate);
                GetXMLField(pSess, "ProcessSaturday", m_szProcessSaturday);
                GetXMLField(pSess, "ProcessSunday", m_szProcessSunday);
                GetXMLField(pSess, "CalendarNumber", m_szCalendarNumber);
                long lOccurs;
                for (lOccurs = 0; lOccurs < 20; lOccurs++)
                {
                    CHolidayTable[lOccurs].Load(pSess);
                }
                CloseXMLRecord(pSess, "SDDATES");
            };
        }
        
        //**************** Start AsdHolidayTable Definition ****************
        //                 HOLIDAY TABLE
        
        class AsdHolidayTable
        {
          private:
            bool      m_bReceived;
            char      m_szSuppressBuff[100];
            bool      m_bHolidayDate;
            char      m_szHolidayDate[8+1];
            bool      m_bHolidayCode;
            char      m_szHolidayCode[1+1];
        
          public:
            long      m_lIndex;

            AsdHolidayTable ()  // Constructor
            {
                m_bReceived = false;
                m_lIndex = -1;
                m_bHolidayDate = false;
                strcpy(m_szHolidayDate, "");
                m_bHolidayCode = false;
                strcpy(m_szHolidayCode, "");
            ...
            //    HOLIDAY DATE
            long lHolidayDate () { return 8; };
            RWCString HolidayDate (void)
            {
                return m_szHolidayDate;
            }
            long HolidayDate (const RWCString& rwc)
            {
                return SetString(rwc, m_szHolidayDate, sizeof(m_szHolidayDate), &m_bHolidayDate);
            }
            
            //    HOLIDAY CODE
            long lHolidayCode () { return 1; };
            RWCString HolidayCode (void)
            {
                return m_szHolidayCode;
            }
            long HolidayCode (const RWCString& rwc)
            {
                return SetString(rwc, m_szHolidayCode, sizeof(m_szHolidayCode), &m_bHolidayCode);
            }
            
            bool HasDataToSave (void)
            {
                if (m_bHolidayDate) return true;
                if (m_bHolidayCode) return true;
                return false;
            }
            ...
            void Save (SESSIONDATA *pSess)
            {
                if (HasDataToSave())
                {
                    AddXMLRecord(pSess, "HolidayTable", m_lIndex);
                    AddXMLField(pSess, "HolidayDate", m_szHolidayDate, &m_bHolidayDate);
                    AddXMLField(pSess, "HolidayCode", m_szHolidayCode, &m_bHolidayCode);
                    AddXMLCloseRecord(pSess, "HolidayTable");  // ACE
                };
            }

            void Load (SESSIONDATA *pSess)
            {
                if (GetXMLRecord(pSess, "HolidayTable", m_lIndex, &m_bReceived))
                {
                    GetXMLField(pSess, "HolidayDate", m_szHolidayDate);
                    GetXMLField(pSess, "HolidayCode", m_szHolidayCode);
                    CloseXMLRecord(pSess, "HolidayTable");
                };
            }
        };

        AsdHolidayTable  CHolidayTable[20];
        AsdHolidayTable *HolidayTable[20];
        
        //**************** End AsdHolidayTable Definition ****************
...
//**************** End AsdTWPCDATEData Definition ****************

typedef AsdTWPCDATEData *LPTWPCDATE;
typedef AsdTWPCDATEData::AsdTSHeader *LPTWPCDATE_TSHeader;
typedef AsdTWPCDATEData::AsdSDDATES *LPTWPCDATE_SDDATES;
typedef AsdTWPCDATEData::AsdSDDATES::AsdHolidayTable *LPTWPCDATE_SDDATES_HolidayTable;
typedef AsdTWPCDATEData::AsdTSMDA *LPTWPCDATE_TSMDA;
typedef AsdTWPCDATEData::AsdTSMDA::AsdMdaTrmMsg *LPTWPCDATE_TSMDA_MdaTrmMsg;
typedef AsdTWPCDATEData::AsdTSMDA::AsdMdaTrmMsg::AsdMdaMsg *LPTWPCDATE_TSMDA_MdaTrmMsg_MdaMsg;
#endif    // AsdTWPCDATEData_H
