#ifndef AsdWVPCDLXCData_H
#define AsdWVPCDLXCData_H

#include <stdlib.h>
#ifdef  AsdWVPCDLXC_INTEGRITY_STRING
#undef  AsdWVPCDLXC_INTEGRITY_STRING
#endif
#define AsdWVPCDLXC_INTEGRITY_STRING        ""

#ifdef  AsdWVPCDLXC_CRC_STRING
#undef  AsdWVPCDLXC_CRC_STRING
#endif
#define AsdWVPCDLXC_CRC_STRING "984640713"
...
//**************** Start AsdWVPCDLXCData Definition ****************
//                 Deluxe Check Order

//                 WVPCDLXCData.h generated by AceGen2 ver. 2.5.09

class AsdWVPCDLXCData
{
  public:
  ...
  public:
    AsdWVPCDLXCData ()  // Constructors
    {
        InitializeSessionData(&m_Sess);                      // InitializeSessionData declared in bcAce2.h
        m_Sess.pFileName = "WVPCDLXC.ace";                       // This variable is fixed at gen time
        m_Sess.pProtocol = "TLU0";                           // This variable is fixed at gen time
        m_Sess.pIntegrityString = AsdWVPCDLXC_INTEGRITY_STRING;  // This variable is fixed at gen time
    ...
    RWCString HostLUName (void)
    {
        ...
        AddXMLField(&m_Sess, m_Sess.pIntegrityTag, AsdWVPCDLXC_INTEGRITY_STRING, NULL);
    ...
    long Execute (void)
    {
        ...
        AddXMLField(&m_Sess, m_Sess.pCRCTag, AsdWVPCDLXC_CRC_STRING, NULL);
        AddXMLRecord(&m_Sess, m_Sess.pDataTag, -1);       // DATA
        CTSHeader.Save(&m_Sess);
        CWVDLXINC.Save(&m_Sess);
        CWVDLXCKG.Save(&m_Sess);
        CTSMDA.Save(&m_Sess);
        ...
        if ((ret = ExecuteXMLTransaction(&m_Sess)) == 0)
        {
            if (GetXMLRecord(&m_Sess, m_Sess.pAceTag, NULL, NULL))
            {
                ...
                {
                    if(GetXMLRecord(&m_Sess, "WVPCDLXC", NULL, NULL))
                    {
                        CTSHeader.Load(&m_Sess);
                        CWVDLXINC.Load(&m_Sess);
                        CWVDLXCKG.Load(&m_Sess);
                        CTSMDA.Load(&m_Sess);
    
                        CloseXMLRecord(&m_Sess, "WVPCDLXC");
                    }
    ...
    //**************** Start AsdWVDLXINC Definition ****************
    //                 CUSTOMER SERVICE WORKSTATION INPUT TO DELUXE CHECKING       
    ...
    class AsdWVDLXINC
    {
      private:
        ...
        bool      m_bReceived;
        char      m_szSuppressBuff[100];
        bool      m_bFiToDeluxeCk;
        char      m_szFiToDeluxeCk[4000+1];
    
      public:
        long      m_lIndex;

        AsdWVDLXINC ()  // Constructor
        {
            m_bHostInitOverride = false;
            strcpy(m_szHostInitOverride, "");
            m_bReceived = false;
            m_lIndex = -1;
            m_bFiToDeluxeCk = false;
            strcpy(m_szFiToDeluxeCk, "");
        }
        ...
        //    DELUXE CHEXSYSTEM INQUIRY MESSAGE: TO DELUXE
        long lFiToDeluxeCk () { return 4000; };
        RWCString FiToDeluxeCk (void)
        {
            return m_szFiToDeluxeCk;
        }
        long FiToDeluxeCk (const RWCString& rwc)
        {
            return SetString(rwc, m_szFiToDeluxeCk, sizeof(m_szFiToDeluxeCk), &m_bFiToDeluxeCk);
        }
        
        bool HasDataToSave (void)
        {
            if (m_bHostInitOverride) return true;
            if (m_bFiToDeluxeCk) return true;
            return false;
        }
        ...
        void Save (SESSIONDATA *pSess)
        {
            if (HasDataToSave())
            {
                AddXMLRecord(pSess, "WVDLXINC", m_lIndex);
                AddXMLField(pSess, "HostInitOverride", m_szHostInitOverride, &m_bHostInitOverride);
                AddXMLField(pSess, "FiToDeluxeCk", m_szFiToDeluxeCk, &m_bFiToDeluxeCk);
                AddXMLCloseRecord(pSess, "WVDLXINC");  // ACE
            };
        }

        void Load (SESSIONDATA *pSess)
        {
            if (GetXMLRecord(pSess, "WVDLXINC", m_lIndex, &m_bReceived))
            {
                GetXMLField(pSess, "FiToDeluxeCk", m_szFiToDeluxeCk);
                CloseXMLRecord(pSess, "WVDLXINC");
            };
        }
    };

    AsdWVDLXINC CWVDLXINC;
    AsdWVDLXINC *WVDLXINC;
    
    //**************** End AsdWVDLXINC Definition ****************

    
    //**************** Start AsdWVDLXCKG Definition ****************
    //                 CUSTOMER SERVICE WORKSTATION OUTPUT FROM DELUXE CHECKING    
    ...
    class AsdWVDLXCKG
    {
      private:
        ...
        bool      m_bReceived;
        char      m_szSuppressBuff[100];
        bool      m_bDeluxeCkToFi;
        char      m_szDeluxeCkToFi[4000+1];
    
      public:
        long      m_lIndex;

        AsdWVDLXCKG ()  // Constructor
        {
            m_bHostInitOverride = false;
            strcpy(m_szHostInitOverride, "");
            m_bReceived = false;
            m_lIndex = -1;
            m_bDeluxeCkToFi = false;
            strcpy(m_szDeluxeCkToFi, "");
        }
        ...
        //    DELUXE CHEXSYSTEM RESPONSE MESSAGE: FROM DELUXE
        long lDeluxeCkToFi () { return 4000; };
        RWCString DeluxeCkToFi (void)
        {
            return m_szDeluxeCkToFi;
        }
        long DeluxeCkToFi (const RWCString& rwc)
        {
            return SetString(rwc, m_szDeluxeCkToFi, sizeof(m_szDeluxeCkToFi), &m_bDeluxeCkToFi);
        }
        
        bool HasDataToSave (void)
        {
            if (m_bHostInitOverride) return true;
            if (m_bDeluxeCkToFi) return true;
            return false;
        }
        ...
        void Save (SESSIONDATA *pSess)
        {
            if (HasDataToSave())
            {
                AddXMLRecord(pSess, "WVDLXCKG", m_lIndex);
                AddXMLField(pSess, "HostInitOverride", m_szHostInitOverride, &m_bHostInitOverride);
                AddXMLField(pSess, "DeluxeCkToFi", m_szDeluxeCkToFi, &m_bDeluxeCkToFi);
                AddXMLCloseRecord(pSess, "WVDLXCKG");  // ACE
            };
        }

        void Load (SESSIONDATA *pSess)
        {
            if (GetXMLRecord(pSess, "WVDLXCKG", m_lIndex, &m_bReceived))
            {
                GetXMLField(pSess, "DeluxeCkToFi", m_szDeluxeCkToFi);
                CloseXMLRecord(pSess, "WVDLXCKG");
            };
        }
    };

    AsdWVDLXCKG CWVDLXCKG;
    AsdWVDLXCKG *WVDLXCKG;
    
    //**************** End AsdWVDLXCKG Definition ****************
...
//**************** End AsdWVPCDLXCData Definition ****************

typedef AsdWVPCDLXCData *LPWVPCDLXC;
typedef AsdWVPCDLXCData::AsdTSHeader *LPWVPCDLXC_TSHeader;
typedef AsdWVPCDLXCData::AsdWVDLXINC *LPWVPCDLXC_WVDLXINC;
typedef AsdWVPCDLXCData::AsdWVDLXCKG *LPWVPCDLXC_WVDLXCKG;
typedef AsdWVPCDLXCData::AsdTSMDA *LPWVPCDLXC_TSMDA;
typedef AsdWVPCDLXCData::AsdTSMDA::AsdMdaTrmMsg *LPWVPCDLXC_TSMDA_MdaTrmMsg;
typedef AsdWVPCDLXCData::AsdTSMDA::AsdMdaTrmMsg::AsdMdaMsg *LPWVPCDLXC_TSMDA_MdaTrmMsg_MdaMsg;
#endif    // AsdWVPCDLXCData_H
