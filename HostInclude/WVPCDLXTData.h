#ifndef AsdWVPCDLXTData_H
#define AsdWVPCDLXTData_H

#include <stdlib.h>
#ifdef  AsdWVPCDLXT_INTEGRITY_STRING
#undef  AsdWVPCDLXT_INTEGRITY_STRING
#endif
#define AsdWVPCDLXT_INTEGRITY_STRING        ""

#ifdef  AsdWVPCDLXT_CRC_STRING
#undef  AsdWVPCDLXT_CRC_STRING
#endif
#define AsdWVPCDLXT_CRC_STRING "763039538"
...
//**************** Start AsdWVPCDLXTData Definition ****************
//                 Deluxe Interface

//                 WVPCDLXTData.h generated by AceGen2 ver. 2.5.09

class AsdWVPCDLXTData
{
  public:
  ...
  public:
    AsdWVPCDLXTData ()  // Constructors
    {
        InitializeSessionData(&m_Sess);                      // InitializeSessionData declared in bcAce2.h
        m_Sess.pFileName = "WVPCDLXT.ace";                       // This variable is fixed at gen time
        m_Sess.pProtocol = "TLU0";                           // This variable is fixed at gen time
        m_Sess.pIntegrityString = AsdWVPCDLXT_INTEGRITY_STRING;  // This variable is fixed at gen time
    ...
    RWCString HostLUName (void)
    {
        ...
        AddXMLField(&m_Sess, m_Sess.pIntegrityTag, AsdWVPCDLXT_INTEGRITY_STRING, NULL);
    ...
    long Execute (void)
    {
        ...
        AddXMLField(&m_Sess, m_Sess.pCRCTag, AsdWVPCDLXT_CRC_STRING, NULL);
        AddXMLRecord(&m_Sess, m_Sess.pDataTag, -1);       // DATA
        CTSHeader.Save(&m_Sess);
        CWVDLXIN.Save(&m_Sess);
        CWVDLXOUT.Save(&m_Sess);
        CTSMDA.Save(&m_Sess);
        ...
        if ((ret = ExecuteXMLTransaction(&m_Sess)) == 0)
        {
            if (GetXMLRecord(&m_Sess, m_Sess.pAceTag, NULL, NULL))
            {
                ...
                {
                    if(GetXMLRecord(&m_Sess, "WVPCDLXT", NULL, NULL))
                    {
                        CTSHeader.Load(&m_Sess);
                        CWVDLXIN.Load(&m_Sess);
                        CWVDLXOUT.Load(&m_Sess);
                        CTSMDA.Load(&m_Sess);
    
                        CloseXMLRecord(&m_Sess, "WVPCDLXT");
                    }
    ...
    //**************** Start AsdWVDLXIN Definition ****************
    //                 CUSTOMER SERVICE WORKSTATION INPUT TO DELUXE                
    ...
    class AsdWVDLXIN
    {
      private:
        ...
        char      m_szSuppressBuff[100];
        bool      m_bFiToDeluxe;
        char      m_szFiToDeluxe[3000+1];
    
      public:
        long      m_lIndex;

        AsdWVDLXIN ()  // Constructor
        {
            m_bHostInitOverride = false;
            strcpy(m_szHostInitOverride, "");
            m_bReceived = false;
        ...
        long lFiToDeluxe () { return 3000; };
        RWCString FiToDeluxe (void)
        {
            return m_szFiToDeluxe;
        }
        long FiToDeluxe (const RWCString& rwc)
        {
            return SetString(rwc, m_szFiToDeluxe, sizeof(m_szFiToDeluxe), &m_bFiToDeluxe);
        }
        
        bool HasDataToSave (void)
        {
            if (m_bHostInitOverride) return true;
            if (m_bFiToDeluxe) return true;
            return false;
        }
        ...
        void Save (SESSIONDATA *pSess)
        {
            if (HasDataToSave())
            {
                AddXMLRecord(pSess, "WVDLXIN", m_lIndex);
                AddXMLField(pSess, "HostInitOverride", m_szHostInitOverride, &m_bHostInitOverride);
                AddXMLField(pSess, "FiToDeluxe", m_szFiToDeluxe, &m_bFiToDeluxe);
                AddXMLCloseRecord(pSess, "WVDLXIN");  // ACE
            };
        }

        void Load (SESSIONDATA *pSess)
        {
            if (GetXMLRecord(pSess, "WVDLXIN", m_lIndex, &m_bReceived))
            {
                GetXMLField(pSess, "FiToDeluxe", m_szFiToDeluxe);
                CloseXMLRecord(pSess, "WVDLXIN");
            };
        }
    };
    ...
    //**************** End AsdWVDLXIN Definition ****************

    
    //**************** Start AsdWVDLXOUT Definition ****************
    //                 CUSTOMER SERVICE WORKSTATION OUTPUT FROM DELUXE             
    ...
    class AsdWVDLXOUT
    {
      private:
        ...
        char      m_szSuppressBuff[100];
        bool      m_bDeluxeToFi;
        char      m_szDeluxeToFi[7200+1];
    
      public:
        long      m_lIndex;

        AsdWVDLXOUT ()  // Constructor
        {
            m_bHostInitOverride = false;
            strcpy(m_szHostInitOverride, "");
            m_bReceived = false;
        ...
        long lDeluxeToFi () { return 7200; };
        RWCString DeluxeToFi (void)
        {
            return m_szDeluxeToFi;
        }
        long DeluxeToFi (const RWCString& rwc)
        {
            return SetString(rwc, m_szDeluxeToFi, sizeof(m_szDeluxeToFi), &m_bDeluxeToFi);
        }
        
        bool HasDataToSave (void)
        {
            if (m_bHostInitOverride) return true;
            if (m_bDeluxeToFi) return true;
            return false;
        }
        ...
        void Save (SESSIONDATA *pSess)
        {
            if (HasDataToSave())
            {
                AddXMLRecord(pSess, "WVDLXOUT", m_lIndex);
                AddXMLField(pSess, "HostInitOverride", m_szHostInitOverride, &m_bHostInitOverride);
                AddXMLField(pSess, "DeluxeToFi", m_szDeluxeToFi, &m_bDeluxeToFi);
                AddXMLCloseRecord(pSess, "WVDLXOUT");  // ACE
            };
        }

        void Load (SESSIONDATA *pSess)
        {
            if (GetXMLRecord(pSess, "WVDLXOUT", m_lIndex, &m_bReceived))
            {
                GetXMLField(pSess, "DeluxeToFi", m_szDeluxeToFi);
                CloseXMLRecord(pSess, "WVDLXOUT");
            };
        }
    };

    AsdWVDLXOUT CWVDLXOUT;
    AsdWVDLXOUT *WVDLXOUT;
    
    //**************** End AsdWVDLXOUT Definition ****************
...
//**************** End AsdWVPCDLXTData Definition ****************

typedef AsdWVPCDLXTData *LPWVPCDLXT;
typedef AsdWVPCDLXTData::AsdTSHeader *LPWVPCDLXT_TSHeader;
typedef AsdWVPCDLXTData::AsdWVDLXIN *LPWVPCDLXT_WVDLXIN;
typedef AsdWVPCDLXTData::AsdWVDLXOUT *LPWVPCDLXT_WVDLXOUT;
typedef AsdWVPCDLXTData::AsdTSMDA *LPWVPCDLXT_TSMDA;
typedef AsdWVPCDLXTData::AsdTSMDA::AsdMdaTrmMsg *LPWVPCDLXT_TSMDA_MdaTrmMsg;
typedef AsdWVPCDLXTData::AsdTSMDA::AsdMdaTrmMsg::AsdMdaMsg *LPWVPCDLXT_TSMDA_MdaTrmMsg_MdaMsg;
#endif    // AsdWVPCDLXTData_H
