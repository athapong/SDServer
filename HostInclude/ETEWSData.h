#ifndef AsdETEWSData_H
#define AsdETEWSData_H

#include <stdlib.h>
#ifdef  AsdETEWS_INTEGRITY_STRING
#undef  AsdETEWS_INTEGRITY_STRING
#endif
#define AsdETEWS_INTEGRITY_STRING        ""

#ifdef  AsdETEWS_CRC_STRING
#undef  AsdETEWS_CRC_STRING
#endif
#define AsdETEWS_CRC_STRING "34981006067"

#include <rw/cstring.h>

#include <AceUserIncludesExit.h>

#include <bcEteGateway.h>

//**************** Start AsdETEWSData Definition ****************
//                 To perform converted transaction via ETE

//                 ETEWSData.h generated by Ath #Jan 15, 2017
...
private:
	...
	ETEGATEWAYSESSIONDATA m_Sess;    // The one and only instance of SessionData

public:
	AsdETEWSData()  // Constructors
	{
		InitializeSessionData(&m_Sess); // InitializeBPSessionData declared in bcEteGateway.h
		m_Sess.pFileName = "EteGateway";    // This variable is fixed at gen time
		m_Sess.pProtocol = "TLU0";         // This variable is fixed at gen time
		m_Sess.pIntegrityString = AsdETEWS_INTEGRITY_STRING; // This variable is fixed at gen time
		strcpy(m_Sess.szSession, "");      // This variable is fixed at gen time

		TSHeader = &CTSHeader;
	}

	~AsdETEWSData()  // Destructor
	{
	...
	RWCString EteGatewayServerIP(void) {
		return m_Sess.szEteGatewayServerIP;
	}
	long EteGatewayServerIP(const RWCString& rwc) {
		return SetString(rwc, m_Sess.szEteGatewayServerIP, sizeof(m_Sess.szEteGatewayServerIP));
	}
	...
	RWCString SafeCode(void) {
		return m_Sess.szSafeCode;
	}
	long SafeCode(const RWCString& rwc) {
		return SetString(rwc, m_Sess.szSafeCode, sizeof(m_Sess.szSafeCode) + 1);
	}

	RWCString XMLHostInput(void) {
		return m_Sess.abSend.Get();
	}
	long XMLHostInput(const RWCString& rwc) {
		return SetString(rwc, m_Sess.abSend.Get(), rwc.length() + 1);
	}

	RWCString XMLHostOutputFull(void) {
		return m_Sess.abRecv.Get();
	}

	RWCString XMLHostOutput(void) {
		char* OutputBuff;
		OutputBuff = (char*)malloc(strlen(m_Sess.abRecv.Get()));
		// extract data from <OUT_TEXT></OUT_TEXT>
		GetEteGatewayOutput(m_Sess.abRecv.Get(), OutputBuff);
		return OutputBuff;
	}

	RWCString XMLHostResponseCode(void) {
		char* OutputBuff;
		OutputBuff = (char*) malloc(4+1);
		GetEteGatewayResponseCode(m_Sess.abRecv.Get(), OutputBuff);
		return OutputBuff;
	}

	RWCString HostLUName(void) {
		AddXMLRecord(&m_Sess, NULL, -1);  // NULL name initializes the buffers
		AddXMLRecord(&m_Sess, m_Sess.pEteGatewayRequestTag, -1);  // ETE Gateway
		...
		AddXMLField(&m_Sess, m_Sess.pIntegrityTag, AsdETEWS_INTEGRITY_STRING, NULL);
		AddXMLCloseRecord(&m_Sess, m_Sess.pEteGatewayRequestTag);  // ETE Gateway
	...
	long Execute(void) {
		long ret;
	#ifdef SERVER_TRACE
		cout << "ETEWSData.Execute() start" << endl;
	#endif
		cout << "DEBUG - ETEWSData.Execute() start" << endl;
		ReadSessionFile(&m_Sess);
		//AddXMLRecord(&m_Sess, NULL, -1);    													// NULL name initializes the buffers
		//m_Sess.abSend.NormalAdd(m_Host);
		cout << "DEBUG - ETEWSData.Execute() ReadSessionFile done!" << endl;
#ifdef SERVER_TRACE
			cout << "m_Sess.abSend: " << m_Sess.abSend.Get() << endl;
#endif
		cout << "DEBUG - ETEWSData.Execute() before ExecuteXMLTransaction()" << endl;
		if ((ret = ExecuteXMLTransaction(&m_Sess)) == 0) {
			cout << "DEBUG - ETEWSData.Execute() after ExecuteXMLTransaction() successful" << endl;
#ifdef SERVER_TRACE
		cout << "m_Sess.abRecv: " << m_Sess.abRecv.Get() << endl;
#endif
		GetXMLField(&m_Sess, m_Sess.pOutMsgCodeTag, m_Sess.szOutMsgCode);
		GetXMLField(&m_Sess, m_Sess.pOutMsgDescTag, m_Sess.szOutMsgDesc);
		GetXMLField(&m_Sess, m_Sess.pOutMsgCodeTag, m_Sess.szStatus);
	}
#ifdef SERVER_TRACE
		cout << "ETEWSData.Execute() end" << endl;
#endif
		return ret;
	}
	...
	long SessionExecute(void) {
		long ret;
#ifdef SERVER_TRACE
		//cout << "ETEWSData.SessionExecute() start" << endl;
#endif
		ReadSessionFile(&m_Sess);
		//AddXMLRecord(&m_Sess, NULL, -1);                  // NULL name initializes the buffers
		//AddXMLRecord(&m_Sess, m_Sess.pHeaderTag, -1);        // Header
		AddXMLRecord(&m_Sess, m_Sess.pEteGatewayRequestTag, -1);        // WEB_EPayment_Request
...
//		AddXMLField(&m_Sess, m_Sess.pTellerIDTag, m_Sess.szTellerID, NULL);
		...
		AddXMLCloseRecord(&m_Sess, m_Sess.pEteGatewayRequestTag); // WEB_EPayment_Request
...
//			cout << "m_Sess.szCustReference: " << g << endl;
...
#ifdef SERVER_TRACE
		//cout << "ETEWSData.SessionExecute() end" << endl;
#endif
		return ret;
	}
	...
	long FreeSession(void) {
		long ret;
		// TODO: update this once complete the main function above
		ReadSessionFile(&m_Sess);
		//AddXMLRecord(&m_Sess, NULL, -1);    // NULL name initializes the buffers
		AddXMLRecord(&m_Sess, m_Sess.pEteGatewayRequestTag, -1);    // TMBInput
		...
		//AddXMLField(&m_Sess, m_Sess.pCRCTag, AsdETEWS_CRC_STRING, NULL);
		AddXMLRecord(&m_Sess, m_Sess.pDataTag, -1);       // DATA

		AddXMLCloseRecord(&m_Sess, m_Sess.pDataTag);           // DATA
		AddXMLCloseRecord(&m_Sess, m_Sess.pEteGatewayRequestTag);   // BP Gateway

		if ((ret = ExecuteXMLTransaction(&m_Sess)) == 0) {
			if (GetXMLRecord(&m_Sess, m_Sess.pEteGatewayResponseTag, NULL, NULL)) {
				...
				CloseXMLRecord(&m_Sess, m_Sess.pEteGatewayResponseTag); // BP Gateway
			}
		}
		m_Sess.lAuthCode = 0;
		m_Sess.lHandle = 0;
	...
	public:
		...
		void HostInitOverride(InitBPValue iv, short custom = 0) {
			EteGatewayHostInitOverride(m_szHostInitOverride, &m_bHostInitOverride, iv, custom);
		}
		...
		bool HasDataToSave(void) {
			...
			if (m_bExtTranCode)
				return true;
			if (m_bOverrideTellerId)
				return true;
			if (m_bTranSeq)
				return true;
			long lOccurs;
			for (lOccurs = 0; lOccurs < 3; lOccurs++)
				if (m_bTranStatus[lOccurs])
					return true;
			if (m_bTranTime)
				return true;
			if (m_bLandFill0)
			...
			if (m_bUserId)
				return true;
			if (m_bFunSecurityCode)
				return true;
			if (m_bBusModuleType)
				return true;
			if (m_bBusinessModule)
				return true;
			if (m_bMaximumSeverity)
				return true;
			if (m_bAppcErrorCode)
				return true;
			if (m_bNumDags)
		...
		void Save(ETEGATEWAYSESSIONDATA *pSess) {
			if (HasDataToSave()) {
		...
		void Load(ETEGATEWAYSESSIONDATA *pSess) {
...
typedef AsdETEWSData *LPETEWS;
typedef AsdETEWSData::AsdTSHeader *LPETEWS_TSHeader;
#endif    // AsdETEWSData_H
