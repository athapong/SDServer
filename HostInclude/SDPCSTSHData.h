#ifndef AsdSDPCSTSHData_H
#define AsdSDPCSTSHData_H

#include <stdlib.h>
#ifdef  AsdSDPCSTSH_INTEGRITY_STRING
#undef  AsdSDPCSTSH_INTEGRITY_STRING
#endif
#define AsdSDPCSTSH_INTEGRITY_STRING        ""

#ifdef  AsdSDPCSTSH_CRC_STRING
#undef  AsdSDPCSTSH_CRC_STRING
#endif
#define AsdSDPCSTSH_CRC_STRING "4157192246"
...
//**************** Start AsdSDPCSTSHData Definition ****************
//                 ST Stop/Hold Inquiry

//                 SDPCSTSHData.h generated by AceGen2 ver. 2.5.09

class AsdSDPCSTSHData
{
  public:
  ...
  public:
    AsdSDPCSTSHData ()  // Constructors
    {
        InitializeSessionData(&m_Sess);                      // InitializeSessionData declared in bcAce2.h
        m_Sess.pFileName = "SDPCSTSH.ace";                       // This variable is fixed at gen time
        m_Sess.pProtocol = "TLU0";                           // This variable is fixed at gen time
        m_Sess.pIntegrityString = AsdSDPCSTSH_INTEGRITY_STRING;  // This variable is fixed at gen time
    ...
    RWCString HostLUName (void)
    {
        ...
        AddXMLField(&m_Sess, m_Sess.pIntegrityTag, AsdSDPCSTSH_INTEGRITY_STRING, NULL);
    ...
    long Execute (void)
    {
        ...
        AddXMLField(&m_Sess, m_Sess.pCRCTag, AsdSDPCSTSH_CRC_STRING, NULL);
        AddXMLRecord(&m_Sess, m_Sess.pDataTag, -1);       // DATA
        CTSHeader.Save(&m_Sess);
        CSDSTCMST.Save(&m_Sess);
        CSDSTSHIQ.Save(&m_Sess);
        CTSMDA.Save(&m_Sess);
        ...
        if ((ret = ExecuteXMLTransaction(&m_Sess)) == 0)
        {
            if (GetXMLRecord(&m_Sess, m_Sess.pAceTag, NULL, NULL))
            {
                ...
                {
                    if(GetXMLRecord(&m_Sess, "SDPCSTSH", NULL, NULL))
                    {
                        CTSHeader.Load(&m_Sess);
                        CSDSTCMST.Load(&m_Sess);
                        CSDSTSHIQ.Load(&m_Sess);
                        CTSMDA.Load(&m_Sess);
    
                        CloseXMLRecord(&m_Sess, "SDPCSTSH");
                    }
    ...
    //**************** Start AsdSDSTCMST Definition ****************
    //                 SD STANDARD COMMUNCATION DAG FOR ST                         
    
    //    SDSTCMST.map
    //    generated by Dsm2Ace ver. 3.0.04
    //    on 07/04/02 16:17:02
    class AsdSDSTCMST
    {
      private:
        bool      m_bHostInitOverride;
        ...
        bool      m_bSelStartSeqNbr;
        char      m_szSelStartSeqNbr[6+1];
    
      public:
        long      m_lIndex;

        AsdSDSTCMST ()  // Constructor
        {
            m_bHostInitOverride = false;
            strcpy(m_szHostInitOverride, "");
            m_bReceived = false;
            ...
            m_bSelStartSeqNbr = false;
            strcpy(m_szSelStartSeqNbr, "+00000");
        }
        ...
        //    SELECTION START SEQUENCE NUMBER
        long lSelStartSeqNbr () { return 6; };
        RWCString SelStartSeqNbr (void)
        {
            return AceZeroSupp(m_szSelStartSeqNbr, &m_szSuppressBuff[0]);
        }
        RWCString SelStartSeqNbrRaw (void)
        {
            return m_szSelStartSeqNbr;
        }
        long SelStartSeqNbr (const RWCString& rwc)
        {
            return SetString(rwc, m_szSelStartSeqNbr, sizeof(m_szSelStartSeqNbr), &m_bSelStartSeqNbr);
        }
        ...
        bool HasDataToSave (void)
        {
            ...
            if (m_bSelStartSeqNbr) return true;
            return false;
        }
        ...
        void Save (SESSIONDATA *pSess)
        {
            if (HasDataToSave())
            {
                AddXMLRecord(pSess, "SDSTCMST", m_lIndex);
                AddXMLField(pSess, "HostInitOverride", m_szHostInitOverride, &m_bHostInitOverride);
                ...
                AddXMLField(pSess, "SelStartSeqNbr", m_szSelStartSeqNbr, &m_bSelStartSeqNbr);
                AddXMLCloseRecord(pSess, "SDSTCMST");  // ACE
            };
        }

        void Load (SESSIONDATA *pSess)
        {
            if (GetXMLRecord(pSess, "SDSTCMST", m_lIndex, &m_bReceived))
            {
                ...
                GetXMLField(pSess, "SelStartSeqNbr", m_szSelStartSeqNbr);
                CloseXMLRecord(pSess, "SDSTCMST");
            };
        }
    };

    AsdSDSTCMST CSDSTCMST;
    AsdSDSTCMST *SDSTCMST;
    
    //**************** End AsdSDSTCMST Definition ****************

    
    //**************** Start AsdSDSTSHIQ Definition ****************
    //                 SD STOP/HOLD INQUIRY FOR ST                                 
    
    //    SDSTSHIQ.map
    //    generated by Dsm2Ace ver. 3.0.04
    //    on 07/04/02 15:39:10
    class AsdSDSTSHIQ
    {
      private:
        bool      m_bHostInitOverride;
        ...
        char      m_szSelStartSeqNbr[6+1];
        bool      m_bStShOccUsed;
        char      m_szStShOccUsed[4+1];
    
      public:
        long      m_lIndex;

        AsdSDSTSHIQ ()  // Constructor
        {
            m_bHostInitOverride = false;
            strcpy(m_szHostInitOverride, "");
            m_bReceived = false;
            ...
            m_bSelStartSeqNbr = false;
            strcpy(m_szSelStartSeqNbr, "+00000");
            m_bStShOccUsed = false;
            strcpy(m_szStShOccUsed, "+000");
            long lOccurs;
        
            for (lOccurs = 0; lOccurs < 50; lOccurs++)
            {
                StShItem[lOccurs]  = &CStShItem[lOccurs];
                CStShItem[lOccurs].m_lIndex = lOccurs;
            }
        }
        ...
        //    NUMBER OF STOP/HOLD OCCURANCES
        long lStShOccUsed () { return 4; };
        RWCString StShOccUsed (void)
        {
            return AceZeroSupp(m_szStShOccUsed, &m_szSuppressBuff[0]);
        }
        RWCString StShOccUsedRaw (void)
        {
            return m_szStShOccUsed;
        }
        long StShOccUsed (const RWCString& rwc)
        {
            return SetString(rwc, m_szStShOccUsed, sizeof(m_szStShOccUsed), &m_bStShOccUsed);
        }
        ...
        bool HasDataToSave (void)
        {
            ...
            if (m_bProcessFlag) return true;
            if (m_bSelStartSeqNbr) return true;
            if (m_bStShOccUsed) return true;
            long lOccurs;
            for (lOccurs = 0; lOccurs < 50; lOccurs++)
            {
                if(CStShItem[lOccurs].HasDataToSave())
                {
                    return true;
                }
            }
        ...
        void Save (SESSIONDATA *pSess)
        {
            if (HasDataToSave())
            {
                AddXMLRecord(pSess, "SDSTSHIQ", m_lIndex);
                AddXMLField(pSess, "HostInitOverride", m_szHostInitOverride, &m_bHostInitOverride);
                AddXMLField(pSess, "ProcessFlag", m_szProcessFlag, &m_bProcessFlag);
                AddXMLField(pSess, "SelStartSeqNbr", m_szSelStartSeqNbr, &m_bSelStartSeqNbr);
                AddXMLField(pSess, "StShOccUsed", m_szStShOccUsed, &m_bStShOccUsed);
                long lOccurs;
                for (lOccurs = 0; lOccurs < 50; lOccurs++)
                {
                    CStShItem[lOccurs].Save(pSess);
                }
                AddXMLCloseRecord(pSess, "SDSTSHIQ");  // ACE
            };
        }

        void Load (SESSIONDATA *pSess)
        {
            if (GetXMLRecord(pSess, "SDSTSHIQ", m_lIndex, &m_bReceived))
            {
                GetXMLField(pSess, "ProcessFlag", m_szProcessFlag);
                GetXMLField(pSess, "SelStartSeqNbr", m_szSelStartSeqNbr);
                GetXMLField(pSess, "StShOccUsed", m_szStShOccUsed);
                long lOccurs;
                for (lOccurs = 0; lOccurs < 50; lOccurs++)
                {
                    CStShItem[lOccurs].Load(pSess);
                }
                CloseXMLRecord(pSess, "SDSTSHIQ");
            };
        }
        
        //**************** Start AsdStShItem Definition ****************
        //                 STOP HOLD ITEM OCCURANCE
        ...
        class AsdStShItem
        {
          private:
            ...
            char      m_szSuppressBuff[100];
            bool      m_bStShSeq;
            char      m_szStShSeq[6+1];
            bool      m_bStShType;
            char      m_szStShType[1+1];
            bool      m_bStShAmt;
            char      m_szStShAmt[13+1];
            bool      m_bStShSerial;
            char      m_szStShSerial[10+1];
            bool      m_bStShHighSerial;
            char      m_szStShHighSerial[10+1];
            bool      m_bStShSubType;
            char      m_szStShSubType[1+1];
            bool      m_bStShInstr;
            char      m_szStShInstr[26+1];
            bool      m_bStShExpCent;
            char      m_szStShExpCent[2+1];
            bool      m_bStShExpYr;
            char      m_szStShExpYr[2+1];
            bool      m_bStShExpMo;
            char      m_szStShExpMo[2+1];
            bool      m_bStShExpDa;
            char      m_szStShExpDa[2+1];
            bool      m_bStShOrgCent;
            char      m_szStShOrgCent[2+1];
            bool      m_bStShOrgYr;
            char      m_szStShOrgYr[2+1];
            bool      m_bStShOrgMo;
            char      m_szStShOrgMo[2+1];
            bool      m_bStShOrgDa;
            char      m_szStShOrgDa[2+1];
            bool      m_bStShOnlineDelIndi;
            char      m_szStShOnlineDelIndi[1+1];
            bool      m_bStShSource;
            char      m_szStShSource[1+1];
            bool      m_bStShOperId;
            char      m_szStShOperId[8+1];
            bool      m_bStShTermId;
            char      m_szStShTermId[4+1];
            bool      m_bStShDate;
            char      m_szStShDate[8+1];
            bool      m_bStShTime;
            char      m_szStShTime[8+1];
        
          public:
            long      m_lIndex;

            AsdStShItem ()  // Constructor
            {
                m_bReceived = false;
                m_lIndex = -1;
                m_bStShSeq = false;
                strcpy(m_szStShSeq, "+00000");
                m_bStShType = false;
                strcpy(m_szStShType, "");
                m_bStShAmt = false;
                strcpy(m_szStShAmt, "+000000000.00");
                m_bStShSerial = false;
                strcpy(m_szStShSerial, "");
                m_bStShHighSerial = false;
                strcpy(m_szStShHighSerial, "");
                m_bStShSubType = false;
                strcpy(m_szStShSubType, "");
                m_bStShInstr = false;
                strcpy(m_szStShInstr, "");
                m_bStShExpCent = false;
                strcpy(m_szStShExpCent, "");
                m_bStShExpYr = false;
                strcpy(m_szStShExpYr, "");
                m_bStShExpMo = false;
                strcpy(m_szStShExpMo, "");
                m_bStShExpDa = false;
                strcpy(m_szStShExpDa, "");
                m_bStShOrgCent = false;
                strcpy(m_szStShOrgCent, "");
                m_bStShOrgYr = false;
                strcpy(m_szStShOrgYr, "");
                m_bStShOrgMo = false;
                strcpy(m_szStShOrgMo, "");
                m_bStShOrgDa = false;
                strcpy(m_szStShOrgDa, "");
                m_bStShOnlineDelIndi = false;
                strcpy(m_szStShOnlineDelIndi, "");
                m_bStShSource = false;
                strcpy(m_szStShSource, "");
                m_bStShOperId = false;
                strcpy(m_szStShOperId, "");
                m_bStShTermId = false;
                strcpy(m_szStShTermId, "");
                m_bStShDate = false;
                strcpy(m_szStShDate, "");
                m_bStShTime = false;
                strcpy(m_szStShTime, "+0000000");
            ...
            //    SEQUENCE NUMBER OF THE ACTUAL RECORD ON FILE
            long lStShSeq () { return 6; };
            RWCString StShSeq (void)
            {
                return AceZeroSupp(m_szStShSeq, &m_szSuppressBuff[0]);
            }
            RWCString StShSeqRaw (void)
            {
                return m_szStShSeq;
            }
            long StShSeq (const RWCString& rwc)
            {
                return SetString(rwc, m_szStShSeq, sizeof(m_szStShSeq), &m_bStShSeq);
            }
            
            //    TYPE OF TRANSACTION
            long lStShType () { return 1; };
            RWCString StShType (void)
            {
                return m_szStShType;
            }
            long StShType (const RWCString& rwc)
            {
                return SetString(rwc, m_szStShType, sizeof(m_szStShType), &m_bStShType);
            }
            
            //    AMOUNT OF THE TRANSACTION
            long lStShAmt () { return 13; };
            RWCString StShAmt (void)
            {
                return AceZeroSupp(m_szStShAmt, &m_szSuppressBuff[0]);
            }
            RWCString StShAmtRaw (void)
            {
                return m_szStShAmt;
            }
            long StShAmt (const RWCString& rwc)
            {
                return SetString(rwc, m_szStShAmt, sizeof(m_szStShAmt), &m_bStShAmt);
            }
            
            //    LOW SERIAL NUMBER FOR STOP RANGE
            long lStShSerial () { return 10; };
            RWCString StShSerial (void)
            {
                return m_szStShSerial;
            }
            long StShSerial (const RWCString& rwc)
            {
                return SetString(rwc, m_szStShSerial, sizeof(m_szStShSerial), &m_bStShSerial);
            }
            
            //    HIGH SERIAL NUMBER FOR STOP RANGE
            long lStShHighSerial () { return 10; };
            RWCString StShHighSerial (void)
            {
                return m_szStShHighSerial;
            }
            long StShHighSerial (const RWCString& rwc)
            {
                return SetString(rwc, m_szStShHighSerial, sizeof(m_szStShHighSerial), &m_bStShHighSerial);
            }
            
            //    SUB TYPE
            long lStShSubType () { return 1; };
            RWCString StShSubType (void)
            {
                return m_szStShSubType;
            }
            long StShSubType (const RWCString& rwc)
            {
                return SetString(rwc, m_szStShSubType, sizeof(m_szStShSubType), &m_bStShSubType);
            }
            
            //    DESCRIPTION OR SPECIAL INSTRUCTIONS
            long lStShInstr () { return 26; };
            RWCString StShInstr (void)
            {
                return m_szStShInstr;
            }
            long StShInstr (const RWCString& rwc)
            {
                return SetString(rwc, m_szStShInstr, sizeof(m_szStShInstr), &m_bStShInstr);
            }
            
            //    EXPIRATION DATE/CENTURY
            long lStShExpCent () { return 2; };
            RWCString StShExpCent (void)
            {
                return m_szStShExpCent;
            }
            long StShExpCent (const RWCString& rwc)
            {
                return SetString(rwc, m_szStShExpCent, sizeof(m_szStShExpCent), &m_bStShExpCent);
            }
            
            //    EXPIRATION DATE/YEAR
            long lStShExpYr () { return 2; };
            RWCString StShExpYr (void)
            {
                return m_szStShExpYr;
            }
            long StShExpYr (const RWCString& rwc)
            {
                return SetString(rwc, m_szStShExpYr, sizeof(m_szStShExpYr), &m_bStShExpYr);
            }
            
            //    EXPIRATION DATE/MONTH
            long lStShExpMo () { return 2; };
            RWCString StShExpMo (void)
            {
                return m_szStShExpMo;
            }
            long StShExpMo (const RWCString& rwc)
            {
                return SetString(rwc, m_szStShExpMo, sizeof(m_szStShExpMo), &m_bStShExpMo);
            }
            
            //    EXPIRATION DATE/DAY
            long lStShExpDa () { return 2; };
            RWCString StShExpDa (void)
            {
                return m_szStShExpDa;
            }
            long StShExpDa (const RWCString& rwc)
            {
                return SetString(rwc, m_szStShExpDa, sizeof(m_szStShExpDa), &m_bStShExpDa);
            }
            
            //    ORIGINAL DATE OF TRANACTION/CENTURY
            long lStShOrgCent () { return 2; };
            RWCString StShOrgCent (void)
            {
                return m_szStShOrgCent;
            }
            long StShOrgCent (const RWCString& rwc)
            {
                return SetString(rwc, m_szStShOrgCent, sizeof(m_szStShOrgCent), &m_bStShOrgCent);
            }
            
            //    ORIGINAL DATE OF TRANACTION/YEAR
            long lStShOrgYr () { return 2; };
            RWCString StShOrgYr (void)
            {
                return m_szStShOrgYr;
            }
            long StShOrgYr (const RWCString& rwc)
            {
                return SetString(rwc, m_szStShOrgYr, sizeof(m_szStShOrgYr), &m_bStShOrgYr);
            }
            
            //    ORIGINAL DATE OF TRANACTION/MONTH
            long lStShOrgMo () { return 2; };
            RWCString StShOrgMo (void)
            {
                return m_szStShOrgMo;
            }
            long StShOrgMo (const RWCString& rwc)
            {
                return SetString(rwc, m_szStShOrgMo, sizeof(m_szStShOrgMo), &m_bStShOrgMo);
            }
            
            //    ORIGINAL DATE OF TRANACTION/DAY
            long lStShOrgDa () { return 2; };
            RWCString StShOrgDa (void)
            {
                return m_szStShOrgDa;
            }
            long StShOrgDa (const RWCString& rwc)
            {
                return SetString(rwc, m_szStShOrgDa, sizeof(m_szStShOrgDa), &m_bStShOrgDa);
            }
            
            //    INDICATES ONLINE INFORMATION
            long lStShOnlineDelIndi () { return 1; };
            RWCString StShOnlineDelIndi (void)
            {
                return m_szStShOnlineDelIndi;
            }
            long StShOnlineDelIndi (const RWCString& rwc)
            {
                return SetString(rwc, m_szStShOnlineDelIndi, sizeof(m_szStShOnlineDelIndi), &m_bStShOnlineDelIndi);
            }
            
            //    INDICATES IF TRANSACTION IS FROM THE ONLINE SYSTEM
            long lStShSource () { return 1; };
            RWCString StShSource (void)
            {
                return m_szStShSource;
            }
            long StShSource (const RWCString& rwc)
            {
                return SetString(rwc, m_szStShSource, sizeof(m_szStShSource), &m_bStShSource);
            }
            
            //    ONLINE SOURCE OF TRANSACTION/TELLER ID, OPER ID, ATM ID
            long lStShOperId () { return 8; };
            RWCString StShOperId (void)
            {
                return m_szStShOperId;
            }
            long StShOperId (const RWCString& rwc)
            {
                return SetString(rwc, m_szStShOperId, sizeof(m_szStShOperId), &m_bStShOperId);
            }
            
            //    TERMINAL IDENTIFICATION
            long lStShTermId () { return 4; };
            RWCString StShTermId (void)
            {
                return m_szStShTermId;
            }
            long StShTermId (const RWCString& rwc)
            {
                return SetString(rwc, m_szStShTermId, sizeof(m_szStShTermId), &m_bStShTermId);
            }
            
            //    SOURCE DATE
            long lStShDate () { return 8; };
            RWCString StShDate (void)
            {
                return m_szStShDate;
            }
            long StShDate (const RWCString& rwc)
            {
                return SetString(rwc, m_szStShDate, sizeof(m_szStShDate), &m_bStShDate);
            }
            
            //    SOURCE TIME
            long lStShTime () { return 8; };
            RWCString StShTime (void)
            {
                return AceZeroSupp(m_szStShTime, &m_szSuppressBuff[0]);
            }
            RWCString StShTimeRaw (void)
            {
                return m_szStShTime;
            }
            long StShTime (const RWCString& rwc)
            {
                return SetString(rwc, m_szStShTime, sizeof(m_szStShTime), &m_bStShTime);
            }
            
            bool HasDataToSave (void)
            {
                if (m_bStShSeq) return true;
                if (m_bStShType) return true;
                if (m_bStShAmt) return true;
                if (m_bStShSerial) return true;
                if (m_bStShHighSerial) return true;
                if (m_bStShSubType) return true;
                if (m_bStShInstr) return true;
                if (m_bStShExpCent) return true;
                if (m_bStShExpYr) return true;
                if (m_bStShExpMo) return true;
                if (m_bStShExpDa) return true;
                if (m_bStShOrgCent) return true;
                if (m_bStShOrgYr) return true;
                if (m_bStShOrgMo) return true;
                if (m_bStShOrgDa) return true;
                if (m_bStShOnlineDelIndi) return true;
                if (m_bStShSource) return true;
                if (m_bStShOperId) return true;
                if (m_bStShTermId) return true;
                if (m_bStShDate) return true;
                if (m_bStShTime) return true;
                return false;
            }
            ...
            void Save (SESSIONDATA *pSess)
            {
                if (HasDataToSave())
                {
                    AddXMLRecord(pSess, "StShItem", m_lIndex);
                    AddXMLField(pSess, "StShSeq", m_szStShSeq, &m_bStShSeq);
                    AddXMLField(pSess, "StShType", m_szStShType, &m_bStShType);
                    AddXMLField(pSess, "StShAmt", m_szStShAmt, &m_bStShAmt);
                    AddXMLField(pSess, "StShSerial", m_szStShSerial, &m_bStShSerial);
                    AddXMLField(pSess, "StShHighSerial", m_szStShHighSerial, &m_bStShHighSerial);
                    AddXMLField(pSess, "StShSubType", m_szStShSubType, &m_bStShSubType);
                    AddXMLField(pSess, "StShInstr", m_szStShInstr, &m_bStShInstr);
                    AddXMLField(pSess, "StShExpCent", m_szStShExpCent, &m_bStShExpCent);
                    AddXMLField(pSess, "StShExpYr", m_szStShExpYr, &m_bStShExpYr);
                    AddXMLField(pSess, "StShExpMo", m_szStShExpMo, &m_bStShExpMo);
                    AddXMLField(pSess, "StShExpDa", m_szStShExpDa, &m_bStShExpDa);
                    AddXMLField(pSess, "StShOrgCent", m_szStShOrgCent, &m_bStShOrgCent);
                    AddXMLField(pSess, "StShOrgYr", m_szStShOrgYr, &m_bStShOrgYr);
                    AddXMLField(pSess, "StShOrgMo", m_szStShOrgMo, &m_bStShOrgMo);
                    AddXMLField(pSess, "StShOrgDa", m_szStShOrgDa, &m_bStShOrgDa);
                    AddXMLField(pSess, "StShOnlineDelIndi", m_szStShOnlineDelIndi, &m_bStShOnlineDelIndi);
                    AddXMLField(pSess, "StShSource", m_szStShSource, &m_bStShSource);
                    AddXMLField(pSess, "StShOperId", m_szStShOperId, &m_bStShOperId);
                    AddXMLField(pSess, "StShTermId", m_szStShTermId, &m_bStShTermId);
                    AddXMLField(pSess, "StShDate", m_szStShDate, &m_bStShDate);
                    AddXMLField(pSess, "StShTime", m_szStShTime, &m_bStShTime);
                    AddXMLCloseRecord(pSess, "StShItem");  // ACE
                };
            }

            void Load (SESSIONDATA *pSess)
            {
                if (GetXMLRecord(pSess, "StShItem", m_lIndex, &m_bReceived))
                {
                    GetXMLField(pSess, "StShSeq", m_szStShSeq);
                    GetXMLField(pSess, "StShType", m_szStShType);
                    GetXMLField(pSess, "StShAmt", m_szStShAmt);
                    GetXMLField(pSess, "StShSerial", m_szStShSerial);
                    GetXMLField(pSess, "StShHighSerial", m_szStShHighSerial);
                    GetXMLField(pSess, "StShSubType", m_szStShSubType);
                    GetXMLField(pSess, "StShInstr", m_szStShInstr);
                    GetXMLField(pSess, "StShExpCent", m_szStShExpCent);
                    GetXMLField(pSess, "StShExpYr", m_szStShExpYr);
                    GetXMLField(pSess, "StShExpMo", m_szStShExpMo);
                    GetXMLField(pSess, "StShExpDa", m_szStShExpDa);
                    GetXMLField(pSess, "StShOrgCent", m_szStShOrgCent);
                    GetXMLField(pSess, "StShOrgYr", m_szStShOrgYr);
                    GetXMLField(pSess, "StShOrgMo", m_szStShOrgMo);
                    GetXMLField(pSess, "StShOrgDa", m_szStShOrgDa);
                    GetXMLField(pSess, "StShOnlineDelIndi", m_szStShOnlineDelIndi);
                    GetXMLField(pSess, "StShSource", m_szStShSource);
                    GetXMLField(pSess, "StShOperId", m_szStShOperId);
                    GetXMLField(pSess, "StShTermId", m_szStShTermId);
                    GetXMLField(pSess, "StShDate", m_szStShDate);
                    GetXMLField(pSess, "StShTime", m_szStShTime);
                    CloseXMLRecord(pSess, "StShItem");
                };
            }
        };

        AsdStShItem  CStShItem[50];
        AsdStShItem *StShItem[50];
        
        //**************** End AsdStShItem Definition ****************
        
    };

    AsdSDSTSHIQ CSDSTSHIQ;
    AsdSDSTSHIQ *SDSTSHIQ;
    
    //**************** End AsdSDSTSHIQ Definition ****************
...
//**************** End AsdSDPCSTSHData Definition ****************

typedef AsdSDPCSTSHData *LPSDPCSTSH;
typedef AsdSDPCSTSHData::AsdTSHeader *LPSDPCSTSH_TSHeader;
typedef AsdSDPCSTSHData::AsdSDSTCMST *LPSDPCSTSH_SDSTCMST;
typedef AsdSDPCSTSHData::AsdSDSTSHIQ *LPSDPCSTSH_SDSTSHIQ;
typedef AsdSDPCSTSHData::AsdSDSTSHIQ::AsdStShItem *LPSDPCSTSH_SDSTSHIQ_StShItem;
typedef AsdSDPCSTSHData::AsdTSMDA *LPSDPCSTSH_TSMDA;
typedef AsdSDPCSTSHData::AsdTSMDA::AsdMdaTrmMsg *LPSDPCSTSH_TSMDA_MdaTrmMsg;
typedef AsdSDPCSTSHData::AsdTSMDA::AsdMdaTrmMsg::AsdMdaMsg *LPSDPCSTSH_TSMDA_MdaTrmMsg_MdaMsg;
#endif    // AsdSDPCSTSHData_H
